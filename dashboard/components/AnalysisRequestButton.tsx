...
export default function AnalysisRequestButton({ code, year, quarter, hasAnalysis }: {
  ...
  const request = async () => {
    if (state.status === "completed" && hasAnalysis) { router.refresh(); return; }
  ...
  return <div className="mb-4 rounded-lg border border-violet-700/70 bg-violet-950/25 p-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-slate-300">기업 개요·핵심 투자 아이디어·실적 원인·향후 전망·주가 구간별 상승/하락 원인·리스크를 공시와 공개 원문으로 분석한다.</p>
      </div>
