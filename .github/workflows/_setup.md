...
| 이름 | 값 | 뜻 |
|---|---|---|
| `SEASON_MODE` | 기존 값 유지 가능 | 2026-09-05부터 폴링 빈도에는 사용하지 않음 |
...
공시는 매일 09:00~23:30 KST 30분 간격, 야간은 3시간 간격이다. 최근 7일을 재조회한다.
유니버스 배치가 네이버 컨센서스/PER와 정밀 재무를 보충하고, 정기보고서 뒤 실제 5거래일 동안
컨센서스가 바뀐 ★·○만 3단계 LLM을 실행한다. 잠정·정기보고서 1·2단계는 공시 폴러가 맡는다.
별도 `llm_batch`는 자동 스케줄 없이 수동 장애 복구로만 남긴다.
Actions 스케줄은 지연될 수 있다. 실측 1,086종목에서 15분을 넘긴 결과 추적은 60분 상한과 100행 분할 저장을 적용한다.
