# PRD Ref: §9 (발굴 목록 섹터 열) · traps.md T11
"""투자자가 쓰는 섹터명으로 분류 — **순수 함수. 외부 I/O 금지.**

왜 필요한가:
  KRX/통계청 업종명은 **투자 판단에 쓰는 말이 아니다.**
  한미반도체·HPSP가 '특수 목적용 기계 제조업'(93종목)에 들어가 있고,
  두산에너빌리티는 '일반 목적용 기계 제조업'이다. 이 이름으로는
  "지금 반도체가 좋다 / 원전이 좋다"를 볼 수 없다.

분류 원칙:
  1. **제품(`products`)을 먼저 본다.** 업종보다 훨씬 구체적이다
     ('특수 목적용 기계' 안에 반도체장비·디스플레이장비·조선기자재가 다 있다).
  2. 제품으로 못 가리면 업종으로 떨어진다.
  3. **둘 다 못 가리면 `기타`다.** 억지로 끼워 맞추지 않는다 —
     틀린 섹터는 없는 섹터보다 나쁘다(화면에서 잘못된 묶음으로 읽힌다).

★ 규칙 순서가 곧 우선순위다. 위에 있을수록 좁고 확실한 규칙이다.
  '반도체 장비'는 '기계'보다, '원전'은 '전력인프라'보다 먼저 걸려야 한다.

★ 반도체는 투자 사이클이 서로 다른 IDM·소재·부품·장비로만 나눈다.
  단순히 `반도체`라는 말의 위치만 보면 동진쎄미켐 같은 소재사가 IDM이 된다.
  그래서 반도체 맥락이 있는 문장에서는 구체적인 밸류체인 증거어를 먼저 본다.

═══════════════════════════════════════════════════════════════════
2026-08-20 개정 — 라벨 표본 78종목 기준 정확도 70.5% → (아래 실측)
  그때까지 잰 것은 **미분류율뿐이었다.** '틀리게 가린 비율'은 아무도 재지 않았고,
  오분류는 에러를 내지 않아 화면에만 조용히 나타났다(기아 → 방산·우주).
  정답지는 `tests/sector_labels.py`, 감시는 `tests/test_sector_map.py`.

세 가지를 고쳤다. 전부 **일반 메커니즘**이다 — 개별 종목은 박지 않는다(T68).

  ① **위치가 우선순위를 이긴다.** `products`는 본업을 먼저 적는다. 예전에는
     "규칙 순서상 먼저인 섹터"가 이겨서, 뒤에 곁다리로 붙은 한 단어가 본업을
     덮어썼다 — 기아의 '군수차량'(→방산), 이구산업의 '/부동산 임대'(→부동산),
     효성티앤씨의 '…건설, 무역,금융 등'(→건설). 이제 **가장 앞에서 걸린 것**을
     쓰고, 규칙 순서는 **같은 위치일 때의 동점 처리**로만 남는다.

  ② **제외어(`SECTOR_EXCLUDES`).** 짧은 키워드가 더 긴 단어 안에 숨어 있었다.
     안랩의 '바이러스**백신**' → 바이오·제약, POSCO의 '스테인**리스**' → 금융,
     두산퓨얼셀의 '연료**전지**' → 2차전지. 한국어는 띄어쓰기로 못 막으므로
     "이 말이 있으면 이 규칙은 건너뛴다"를 규칙별로 적는다.

  ③ **회사명을 매칭에서 뺐다.** 이름이 업종을 오염시켰다 — '주성**엔지니어링**'이
     건설로 분류됐다. 이름은 사업의 증거가 아니다.
"""

from __future__ import annotations

import re

#: (섹터명, 제품/업종에서 찾을 키워드들). **순서가 우선순위다.**
#: 키워드는 소문자로 비교한다(영문 혼용 대비).
SECTOR_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    # ── 좁고 확실한 것부터 ──────────────────────────────────────
    ("원전", ("원전", "원자력", "원자로", "smr", "핵연료", "방사성")),
    ("방산·우주", ("방산", "방위산업", "군수", "유도무기", "우주선", "위성", "항공기",
                  "탄약", "함정", "전투", "육해공군", "레이더", "열영상", "탐지추적",
                  "감시장비", "국방")),
    ("2차전지", ("2차전지", "이차전지", "리튬", "양극재", "음극재", "분리막", "전해질",
                 "전해액", "배터리", "전지")),
    # ★ 네 하위 섹터의 구체적 증거어는 `_semiconductor_hit`이 먼저 판정한다.
    ("반도체 장비", ("반도체장비", "반도체 장비", "반도체 후공정장비", "반도체 제조용 기계",
                    "후공정장비", "전공정", "식각", "증착", "노광", "cmp", "cvd",
                    "어닐링", "웨이퍼 가공", "테스트핸들러",
                    "본더", "다이서", "반도체 검사", "반도체금형", "드라이스트립",
                    "레이저 마커", "레이저마커", "세정장비", "이온주입", "검사장비",
                    "검사 장비", "테스터", "계측장비", "계측 장비", "이송장치",
                    "이송 장치", "제조장비", "제조 장비", "진공펌프", "진공 펌프",
                    "로봇시스템", "로봇 시스템", "모션시스템", "모션 시스템",
                    "부착장비", "배관설비", "습도제어 system")),
    ("반도체 소재", ("반도체 소재", "반도체소재", "반도체 재료", "반도체재료",
                    "반도체용 재료", "반도체용재료", "반도체 공정 소재", "반도체 공정재료",
                    "반도체 화학재료", "화학재료", "화학소재", "감광제", "현상액",
                    "포토레지스트", "photoresist", "반도체용 가스", "반도체용 특수가스",
                    "반도체 특수가스", "특수가스", "케미컬", "프리커서", "precursor",
                    "전구체", "박막재료", "슬러리", "slurry", "실리콘 웨이퍼", "웨이퍼 제조")),
    ("반도체 부품", ("반도체 부품", "반도체부품", "반도체패키징", "반도체 패키징",
                    "패키지 기판", "패키지기판", "ic 기판", "ic기판", "테스트 소켓",
                    "테스트소켓", "반도체 소켓", "반도체소켓", "소켓", "리노핀", "프로브 카드",
                    "프로브카드", "리드 프레임", "리드프레임", "반도체패키지", "패키지용",
                    "substrate", "인쇄회로기판", "module pcb", "메모리 모듈", "솔더볼",
                    "solderball", "범핑", "bumping", "펠리클", "포커스링", "석영유리",
                    "합성쿼츠", "히터블록", "반도체용링", "반도체 테스트", "반도체테스트",
                    "반도체 신뢰성", "반도체용세금선", "소모성 부품", "장비 부품")),
    ("반도체 IDM", ("반도체", "메모리", "dram", "nand", "낸드", "시스템반도체",
                   "파운드리", "foundry", "집적회로", "hbm", "팹리스", "fabless", "웨이퍼")),
    # ★ PCB·전자부품은 별도 섹터다. 반도체로 묶으면 밸류체인이 뭉개지고,
    #   '기타'로 두면 39종목(미분류 최다)이 통째로 사라진다.
    ("전자부품", ("인쇄회로", "pcb", "fpcb", "mlb", "전자부품", "커넥터", "적층세라믹",
                  "mlcc", "카메라모듈", "컨덴서", "콘덴서", "저항기", "인덕터",
                  "리드프레임", "기판")),
    ("디스플레이", ("디스플레이", "oled", "lcd", "tft", "패널", "편광", "led",
                    "광학기기", "사진장비", "영상 및 음향", "음향기기")),
    # ★ '터어빈'(원문 표기)·'발전설비'가 '선박용엔진'보다 먼저 걸려야 한다 —
    #   두산에너빌리티의 products는 '기관,터어빈,선박용엔진,주단조품…'이라
    #   순서를 안 맞추면 조선·해운으로 분류된다(실측).
    ("전력인프라", ("변압기", "전선", "케이블", "송전", "배전", "변전", "전력기기",
                   "개폐기", "차단기", "전력설비", "발전기", "전력 변환", "에너지저장",
                   "ess", "스마트그리드", "전기 변환", "터빈", "터어빈", "발전설비",
                   "전력자원", "전기업", "가스 공급", "열병합", "냉·온수",
                   "발전소", "수변전")),
    ("신재생에너지", ("태양광", "풍력", "수소", "연료전지", "바이오매스", "폴리실리콘")),
    ("조선·해운", ("조선", "선박", "보트", "해운", "선박용", "조선기자재", "해양플랜트",
                  "lng선", "보냉재")),
    ("자동차", ("자동차", "차량", "승용차", "상용차", "완성차", "타이어", "전장부품",
                "자동차부품", "차부품")),
    ("바이오·제약", ("의약", "제약", "바이오", "신약", "백신", "항체", "세포치료",
                    "cmo", "cdmo", "건강기능식품", "진단시약", "톡신", "필러",
                    "바이오시밀러", "램시마", "치료제", "임상")),
    ("의료기기", ("의료기기", "의료용 기기", "임플란트", "초음파", "진단기기", "치과",
                  "미용기기", "의료용품")),
    # ★ 예전엔 '방송'이 그냥 들어 있어서 **'통신 및 방송 장비 제조업'이 전부
    #   엔터로 갔다**(케이엠더블유·삼지전자). 방송'장비'는 통신·네트워크다.
    ("엔터·미디어", ("엔터테인먼트", "연예", "음반", "음원", "공연", "아티스트", "드라마",
                    "영화", "방송프로그램", "방송사", "방송콘텐츠", "방송 콘텐츠",
                    "콘텐츠", "웹툰", "매니지먼트", "오디오물", "녹음",
                    "출판", "ent.")),
    ("게임", ("게임", "모바일게임", "온라인게임", "게임소프트웨어")),
    ("인터넷·플랫폼", ("포털", "플랫폼", "전자상거래", "이커머스", "인터넷 정보매개",
                      "인터넷 서비스", "인터넷서비스", "광고대행", "커머스")),
    # ★ '바이러스백신'은 여기다. 제약이 아니다(안랩이 바이오·제약으로 갔었다).
    ("소프트웨어·IT", ("소프트웨어", "시스템 통합", "solution", "솔루션", "보안",
                      "클라우드", "erp", "인증서", "데이터베이스", "인공지능",
                      "정보 서비스", "컴퓨터", "주변장치", "프로그래밍",
                      "바이러스백신", "안티바이러스", "it서비스", "it 서비스")),
    ("통신·네트워크", ("통신장비", "통신기기", "방송 장비", "중계기", "기지국", "안테나",
                      "광케이블", "광섬유", "라우터", "전기 통신", "전기통신",
                      "유무선통신", "통신사업", "네트워크장비", "트랜시버")),
    ("건설", ("건설", "토목", "건축", "시공", "플랜트", "주택분양", "엔지니어링")),
    # ★ '철근'을 여기서 뺐다 — 철근을 만드는 곳은 제철소다(현대제철이 건자재였다).
    ("건자재", ("시멘트", "레미콘", "골재", "단열재", "창호", "석고보드",
                "내화물", "도료", "페인트", "유리", "타일", "위생도기", "탱크",
                "구조용 금속", "증기발생기")),
    # ★ 압연·강판류를 넣기 전에는 POSCO홀딩스의 products('…스테인리스…')에서
    #   걸리는 규칙이 **금융의 '리스'뿐**이었다.
    ("철강·금속", ("철강", "제철", "제강", "철근", "열연", "냉연", "강판", "후판",
                  "선재", "형강", "압연", "스테인리스", "스테인레스",
                  "비철금속", "알루미늄", "구리", "동판", "황동", "인청동",
                  "아연", "니켈", "금속 가공", "주조", "단조", "도금")),
    ("화학·소재", ("화학", "석유화학", "합성수지", "플라스틱", "고무", "섬유", "필름",
                  "접착", "촉매", "가스", "펄프", "종이", "판지", "골판지", "포장재")),
    ("기계·로봇", ("로봇", "공작기계", "산업기계", "감속기", "베어링", "펌프", "밸브",
                  "공구", "엔드밀", "드릴", "절삭", "초경", "자동화", "건설기계",
                  "중장비", "equipment", "지게차", "엘리베이터", "에스컬레이터",
                  "컴프레서", "일반 목적용 기계", "특수 목적용 기계")),
    ("식음료", ("식품", "음료", "주류", "제과", "유가공", "축산", "사료", "알코올",
                "급식", "외식", "담배", "홍삼", "낙농", "유지")),
    ("유통·소비재", ("유통", "도매", "소매", "백화점", "편의점", "홈쇼핑", "화장품",
                    "생활용품", "의류", "패션", "신발", "가죽", "가방", "가구", "침대",
                    "잡화", "완구")),
    ("운송·물류", ("물류", "택배", "운송", "항공운송", "터미널", "창고", "해상운송",
                  "logistics", "포워딩", "항만하역", "특송")),
    # ★ '리스'를 뺐다 — '스테인**리스**'에 걸린다. 리스업은 '리스업'으로만 잡는다.
    ("금융", ("은행", "증권", "보험", "카드", "캐피탈", "저축은행", "자산운용",
              "신탁", "금융", "리스업", "할부금융")),
    ("부동산·리츠", ("리츠", "부동산 임대", "부동산임대", "부동산 공급", "임대업")),
    # ★ '광고'를 뺐다 — 카카오의 '인터넷 서비스(인터넷 광고)'가 여기로 왔다.
    #   광고대행업은 인터넷·플랫폼의 '광고대행'이 잡는다.
    ("지주·기타서비스", ("지주", "경영 컨설팅", "회사 본부", "연구개발업", "교육", "여행",
                        "레저", "호텔", "카지노", "숙박", "학원", "경비",
                        "사업지원", "전문, 과학")),
)

#: 규칙별 **제외어** — 이 말이 들어 있으면 그 규칙은 건너뛴다.
#:
#: ★ 한국어는 띄어쓰기로 단어 경계를 못 막는다. 짧은 키워드가 더 긴 단어 안에
#:   숨어 있으면 **에러 없이** 엉뚱한 섹터가 나온다. 아래는 전부 실측으로 잡은 것이다.
SECTOR_EXCLUDES: dict[str, tuple[str, ...]] = {
    # 군수차량을 만드는 곳은 완성차 업체다(기아 → 방산·우주였다).
    "방산·우주": ("군수차량",),
    # '연료전지'·'태양전지'의 '전지'는 2차전지가 아니다(두산퓨얼셀 → 2차전지였다).
    "2차전지": ("연료전지", "태양전지"),
    # 'HPSP: 고압 수소 어닐링 장비'의 수소는 수소경제가 아니라 공정가스다.
    "신재생에너지": ("수소 어닐링", "수소어닐링"),
    # 컴퓨터 바이러스 백신은 제약이 아니다(안랩 → 바이오·제약이었다).
    "바이오·제약": ("바이러스백신", "안티바이러스", "백신 프로그램"),
    # 드릴십은 조선이다.
    "기계·로봇": ("드릴십",),
}

#: **업종 칸에서만** 쓰는 키워드 — 제품 칸에서는 무시한다.
#:
#: ★ 이것들은 제품 이름이 아니라 **KRX 업종 분류 문구**다. 공시 담당자가 제품 칸에
#:   업종명을 그대로 복사해 넣는 일이 잦은데(LG이노텍의 products는 아예
#:   '기타 전자부품 제조업'이다), 그러면 문장 맨 앞에 업종명이 와서
#:   **본업보다 먼저 걸린다.** 삼성전자의 products는 '통신 및 방송 장비 제조(무선)
#:   제품, 반도체 제조(메모리) 제품…'이라 통신·네트워크가 됐다.
#:   업종 문구는 업종 칸에서만 판단 근거로 쓴다.
INDUSTRY_ONLY_KEYWORDS: frozenset[str] = frozenset({
    "방송 장비",
    "일반 목적용 기계",
    "특수 목적용 기계",
    "금속 가공",
    "전문, 과학",
    "전기업",
    "인터넷 정보매개",
    "정보 서비스",
})

#: 분류하지 못한 종목. **억지로 끼워 맞추지 않는다.**
UNKNOWN_SECTOR = "기타"

# ETF의 세부 상품명은 운용사별로 바뀔 수 있으므로 코드에 상품명을 박지 않는다.
# 대신 국내 테마형 ETF에서 공통으로 쓰는 투자 테마 수준으로만 연결한다.
# 분류 자체는 아래 매핑이 아니라 위의 주요 제품·업종 규칙이 결정한다.
SECTOR_ETF_THEMES: dict[str, str] = {
    "원전": "원자력·원전",
    "방산·우주": "방산·우주항공",
    "2차전지": "2차전지",
    "반도체 IDM": "반도체 IDM",
    "반도체 소재": "반도체 소재",
    "반도체 부품": "반도체 부품",
    "반도체 장비": "반도체 장비",
    "전자부품": "전자부품",
    "디스플레이": "디스플레이",
    "전력인프라": "전력기기·전력인프라",
    "신재생에너지": "신재생에너지",
    "조선·해운": "조선",
    "자동차": "자동차",
    "바이오·제약": "헬스케어·바이오",
    "의료기기": "의료기기",
    "엔터·미디어": "미디어·엔터",
    "게임": "게임",
    "인터넷·플랫폼": "인터넷·플랫폼",
    "소프트웨어·IT": "소프트웨어·IT",
    "통신·네트워크": "통신·네트워크",
    "건설": "건설",
    "건자재": "건자재",
    "철강·금속": "철강·금속",
    "화학·소재": "화학·소재",
    "기계·로봇": "기계·로봇",
    "식음료": "음식료",
    "유통·소비재": "소비재",
    "운송·물류": "운송·물류",
    "금융": "금융",
    "부동산·리츠": "리츠·부동산",
    "지주·기타서비스": "지주·서비스",
    UNKNOWN_SECTOR: UNKNOWN_SECTOR,
}

#: 화면 필터에 쓸 전체 목록(규칙 순서 + 기타).
ALL_SECTORS: tuple[str, ...] = tuple(name for name, _ in SECTOR_RULES) + (UNKNOWN_SECTOR,)


def _haystack(*parts: str | None) -> str:
    """비교용 문자열. 소문자 + 공백 정규화."""
    joined = " ".join(p for p in parts if p)
    return re.sub(r"\s+", " ", joined).lower()


#: 반도체 문맥. 소재의 `감광제`처럼 디스플레이에도 쓰이는 단어는 이 문맥이
#: 함께 있을 때만 반도체로 보낸다. 반면 장비 증거어는 기존 실측(HPSP 등) 때문에
#: 단독으로도 충분한 증거로 인정한다.
SEMICONDUCTOR_CONTEXT: tuple[str, ...] = (
    "반도체", "메모리", "dram", "nand", "낸드", "hbm", "파운드리", "foundry",
    "팹리스", "fabless", "집적회로", "웨이퍼",
)
#: 반도체라는 말이 없어도 공정 장비로 볼 수 있는 좁은 증거어. `검사장비`처럼
#: 의료·자동차에도 쓰이는 일반어는 여기에 넣지 않고 반도체 문맥을 요구한다.
SEMICONDUCTOR_EQUIPMENT_STANDALONE: tuple[str, ...] = (
    "후공정장비", "전공정", "식각", "증착", "노광", "cmp", "cvd", "어닐링",
    "웨이퍼 가공", "테스트핸들러", "본더", "다이서", "반도체 검사", "반도체금형",
    "드라이스트립", "레이저 마커", "레이저마커", "세정장비", "이온주입",
)
SEMICONDUCTOR_SPECIFIC_ORDER: tuple[str, ...] = (
    "반도체 장비", "반도체 소재", "반도체 부품",
)
SEMICONDUCTOR_SECTORS: frozenset[str] = frozenset({
    *SEMICONDUCTOR_SPECIFIC_ORDER, "반도체 IDM",
})


def _semiconductor_hit(text: str, *, allow_industry_words: bool) -> str | None:
    """반도체 밸류체인을 구체적 증거어 → IDM 순으로 판정한다.

    이 선판정은 네 하위 섹터 안에서만 위치 우선 원칙을 보완한다. 다른 산업은
    기존처럼 제품에 먼저 적힌 본업이 이긴다.
    """
    if not text:
        return None
    rules = dict(SECTOR_RULES)
    equipment_hits = [
        text.find(keyword) for keyword in rules[SEMICONDUCTOR_SPECIFIC_ORDER[0]]
        if text.find(keyword) >= 0
    ]
    has_context = any(keyword in text for keyword in SEMICONDUCTOR_CONTEXT)
    standalone_equipment = any(
        keyword in text for keyword in SEMICONDUCTOR_EQUIPMENT_STANDALONE
    )
    if not has_context and not standalone_equipment:
        return None

    context_positions = [
        text.find(keyword) for keyword in SEMICONDUCTOR_CONTEXT if text.find(keyword) >= 0
    ]
    anchor = min(equipment_hits + context_positions)
    competitor_positions: list[int] = []
    for sector, keywords in SECTOR_RULES:
        if sector in SEMICONDUCTOR_SECTORS:
            continue
        if any(bad in text for bad in SECTOR_EXCLUDES.get(sector, ())):
            continue
        usable = (
            keywords if allow_industry_words
            else tuple(keyword for keyword in keywords if keyword not in INDUSTRY_ONLY_KEYWORDS)
        )
        competitor_positions.extend(
            pos for pos in (text.find(keyword) for keyword in usable) if pos >= 0
        )
    # 본업으로 먼저 적힌 다른 산업이 있으면 그 정체성을 보존한다(삼성전기 등).
    if competitor_positions and min(competitor_positions) < anchor:
        return None

    # 같은 문장에 장비와 소재·부품이 함께 있으면 무엇을 파는지 더 구체적인
    # 소재·부품 증거가 이긴다(CMP slurry, 장비용 소모성 부품, 검사용 소켓).
    for sector in ("반도체 소재", "반도체 부품"):
        if any(keyword in text for keyword in rules[sector]):
            return sector
    if equipment_hits:
        return SEMICONDUCTOR_SPECIFIC_ORDER[0]
    return "반도체 IDM"


def _first_hit(text: str, *, allow_industry_words: bool) -> str | None:
    """가장 **앞에서** 걸린 섹터. 같은 위치면 규칙 순서가 앞선 쪽.

    ★ 위치를 먼저 보는 이유: `products`는 본업을 앞에 적는다.
      규칙 순서만 보면 뒤에 곁다리로 붙은 한 단어가 본업을 덮어쓴다 —
      '승용차,중대형버스,트럭,민수특수차량,**군수차량**'이 방산이 되는 식이다.

    ★ 규칙 순서가 **동점 처리로 여전히 살아 있다**: '반도체 후공정장비'는
      '반도체'와 0번째에서 같이 걸리는데, 반도체장비가 규칙상 먼저라 이긴다.
    """
    if not text:
        return None
    semiconductor = _semiconductor_hit(text, allow_industry_words=allow_industry_words)
    if semiconductor is not None:
        return semiconductor
    best: tuple[int, int] | None = None
    best_sector: str | None = None
    for order, (sector, keywords) in enumerate(SECTOR_RULES):
        if sector in SEMICONDUCTOR_SECTORS:
            continue  # 네 하위 섹터는 문맥까지 보는 `_semiconductor_hit`만 판정한다.
        if any(bad in text for bad in SECTOR_EXCLUDES.get(sector, ())):
            continue  # 제외어에 걸리면 이 규칙은 없는 셈 친다
        usable = (
            keywords
            if allow_industry_words
            else tuple(k for k in keywords if k not in INDUSTRY_ONLY_KEYWORDS)
        )
        hits = [pos for pos in (text.find(k) for k in usable) if pos >= 0]
        if not hits:
            continue
        candidate = (min(hits), order)
        if best is None or candidate < best:
            best, best_sector = candidate, sector
    return best_sector


def classify_sector(
    name: str | None = None,
    industry: str | None = None,
    products: str | None = None,
) -> str:
    """투자 섹터명을 돌려준다. 못 가리면 `기타`.

    ★ **제품을 업종보다 먼저** 본다. 업종만 보면 '특수 목적용 기계 제조업' 93종목이
      전부 같은 섹터가 되는데, 그 안에 반도체 장비·디스플레이장비·조선기자재가 섞여 있다.

    실측 대조 (2026-08-17, 실제 DB 값 그대로):
      한미반도체   / 특수 목적용 기계  / '반도체 후공정장비,반도체금형…'  → 반도체 장비
      두산에너빌리티 / 일반 목적용 기계  / '기관,터어빈,선박용엔진,주단조품…' → 전력인프라
      셀트리온     / 기초 의약물질    / '램시마, 트룩시마, 허쥬마'        → 바이오·제약
      동성화인텍   / 기초 화학물질    / '초저온 보냉재'                 → 조선·해운

    ★ **분류는 DB에 적힌 것만 본다.** 두산에너빌리티의 products에는 '원자력'이
      한 글자도 없어서 원전으로 분류되지 않는다 — 투자자 인식과는 다를 수 있다.
      개별 종목을 손으로 박아 넣지 않는 이유: 1,322종목을 그렇게 관리할 수 없고,
      박아 넣은 값은 원자료가 바뀌어도 안 따라가 조용히 낡는다.
    """
    # 1차: 제품으로만 판정한다. 가장 구체적인 신호다.
    hit = _first_hit(_haystack(products), allow_industry_words=False)
    if hit is not None:
        return hit

    # 2차: 업종으로 떨어진다.
    #
    # ★ **회사명은 쓰지 않는다.** `name` 인자는 호출부 호환을 위해 남겨 두지만
    #   매칭에는 넣지 않는다 — 이름이 업종을 오염시켰다. '주성**엔지니어링**'이
    #   건설로, '한국**전력**공사'처럼 우연히 맞는 경우와 구분이 안 됐다.
    #   이름은 사업의 증거가 아니다.
    hit = _first_hit(_haystack(industry), allow_industry_words=True)
    if hit is not None:
        return hit

    return UNKNOWN_SECTOR


# ═══════════════════════════════════════════════════════════════════
# DB 반영 — `python -m src.universe.sector_map --save`
#
# ★ 여기서만 I/O를 한다. 위 `classify_sector`는 순수 함수로 남긴다.
# ★ 유니버스를 다시 크롤링하지 않는다 — 이미 DB에 있는 industry/products로
#   충분하고, KRX를 또 때릴 이유가 없다.
# ═══════════════════════════════════════════════════════════════════
def _main() -> int:
    import argparse
    import collections

    from src.db.supabase_client import (
        get_client,
        select_all,
        upsert_tolerating_missing_columns,
    )
    from src.utils.console import enable_utf8_stdout

    enable_utf8_stdout()
    parser = argparse.ArgumentParser(description="투자 섹터 분류 → krx_universe.sector")
    parser.add_argument("--save", action="store_true")
    parser.add_argument("--show", type=int, default=0, help="섹터별 표본 N개 출력")
    args = parser.parse_args()

    rows = select_all("krx_universe", "code,symbol,name,board,industry,products")
    tagged = [
        (r, classify_sector(r.get("name"), r.get("industry"), r.get("products")))
        for r in rows
    ]
    counts = collections.Counter(sector for _, sector in tagged)

    line = "═" * 66
    print(line)
    print(f"투자 섹터 분류 — {len(rows)}종목 → {len(counts)}섹터")
    print(line)
    for sector, n in counts.most_common():
        bar = "█" * max(1, round(n / max(counts.values()) * 28))
        print(f"  {sector:<14}{n:>5}  {bar}")

    unknown = counts.get(UNKNOWN_SECTOR, 0)
    print(f"\n  미분류 {unknown}종목 ({unknown / max(len(rows), 1) * 100:.1f}%)")
    print("  ★ 미분류는 억지로 끼워 맞추지 않은 결과다 — 틀린 섹터보다 낫다.")

    if args.show:
        print("\n섹터별 표본")
        by_sector: dict[str, list[str]] = collections.defaultdict(list)
        for row, sector in tagged:
            by_sector[sector].append(str(row.get("name")))
        for sector, _ in counts.most_common():
            print(f"  {sector:<14}{', '.join(by_sector[sector][: args.show])}")

    if not args.save:
        print("\n(--save 미지정 — DB에 기록하지 않았다)")
        return 0

    # ★ upsert는 NOT NULL 컬럼을 전부 요구한다(symbol·name·board). 빼면 23502로 죽는다.
    payload = [
        {
            "code": r["code"], "symbol": r["symbol"], "name": r["name"],
            "board": r["board"], "sector": sector,
        }
        for r, sector in tagged
    ]
    saved, dropped = upsert_tolerating_missing_columns(
        get_client(), "krx_universe", payload, on_conflict="code"
    )
    print(f"\n✓ krx_universe {saved}행 갱신")
    if dropped:
        print(f"  ⚠ DB에 없는 컬럼을 빼고 저장했다: {', '.join(dropped)}")
        print("    → schema.sql의 `ADD COLUMN sector`를 SQL Editor에 적용하라 (T18).")
        print("    분류는 됐지만 **저장되지 않았다.**")
    return 0


if __name__ == "__main__":
    raise SystemExit(_main())
