...
# 월 LLM 비용 실링.
# ★ 실측(2026-08-17): 캐시히트 $0.0315/건 · 미스 $0.0363/건.
#   게이트 통과 238종목 전부 분석 = 분기 $7.51인데, 실적 시즌이 한 달에 몰리므로
#   **분기 비용 ≈ 그 달 비용**이다. 실링 $8이면 94%로 빠듯하고, 텔레그램 재질의나
#   재분석이 겹치면 월중에 막힌다 → $12로 올린다(238종목 + 여유 140건).
...
#   상한 80 × $0.0315 = 하루 최대 $2.52이므로 실링($12)을 하루에 태울 수 없다.
