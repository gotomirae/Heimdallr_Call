...
P1_HIGH_DRAWDOWN_FLOOR_PCT = -30.0
P2_ANNOUNCEMENT_RETURN_MAX_PCT = 30.0
P3_PER_PREMIUM_MAX_PCT = 50.0
P4_FOREIGN_NET_RATIO_ANCHORS_PCT = (-5.0, 0.0, 5.0)
P5_RSI_ANCHORS = (30.0, 45.0, 70.0)
...
TECHNICAL_POST_ANNOUNCEMENT_CORRECTION_PCT = -20.0
TECHNICAL_MIN_HIGH_DRAWDOWN_PCT = -20.0
TECHNICAL_MIN_SECTOR_CAGR_PCT = 15.0
TECHNICAL_INVESTOR_BUY_STREAK_DAYS = 3
# "하락"은 20거래일 수익률이 음수인 경우로 정의한다.
...
TECHNICAL_SMA_GAP_MAX_ABS_PCT = 2.0
# RSI는 필수 게이트가 아니라 30~45 회복 시 강력 추천 표시만 한다.
TECHNICAL_RSI_STRONG_MAX = 45.0
...
#
# ★★ 2026-08-24 $20 → **$24** (사용자 결정). **종목이 아니라 단가가 올랐다.**
#   공시 발췌를 실으면서 입력이 길어지자 모델 출력도 함께 늘었다 —
#   출력 중앙 **2,911 → 4,125토큰(+42%)** · 건당 **$0.0362 → $0.0628(+73%)**.
...
# ★★ 2026-08-24 재실측 12,288 → **16,384**(T101). 프롬프트를 "구체적으로 쓰라"로
#   바꾸자 출력이 늘었다 — 중앙 2,911 → **4,989**. 9건 중 1건이 상한에 정확히
#   닿아 **잘렸고, 저장은 안 되면서 $0.1336는 나갔다**(T84: 실패한 호출도 돈은 나간다).
#   ★ 상한을 올려도 **평균 비용은 거의 안 오른다** — 모델은 필요한 만큼만 쓰고,
#     중앙값이 상한의 30%뿐이다. 반대로 상한이 낮으면 **낭비가 확정된다.**
#   ★ 16,384는 비스트리밍 HTTP 타임아웃 안에서 안전한 범위다. 더 올리려면
#     `.stream()`으로 바꿔야 한다.
...
#
# ★★ 2026-08-24 재실측 14,000 → **16,000**. 두 가지가 늘었다(T101):
#     시스템 프롬프트  4,573 → ~5,600  (구체성 요구 · 주가 궤적 해석 지침)
#     분기말 종가 13행 + 최근 공시 목록 + 기준일   신규 ~700
#   게이트 통과 14종목 실측: 중앙 13,611 · **최대 13,768**.
#   14,000이면 여유가 232뿐이라 **긴 발췌를 가진 종목이 통째로 실패**한다
