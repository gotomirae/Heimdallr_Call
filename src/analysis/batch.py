...
운영 선정 기준 (2026-09-07):
  ...
  중요한 종목이 먼저 처리되게 한다. 매력도는 기업 점수와 낮은 반영도를 함께 보되
  저장 스코어로 합산하지 않는다(ADR 5) — 정렬 키일 뿐이다.
...
def attractiveness(screen: dict) -> float | None:
    """투자 매력도 정렬 키. **저장 점수가 아니다** — 순서를 정하는 용도다.

    ★ 기업 점수와 반영도는 화면·저장에서 별도 축으로 유지한다(ADR 5).
    ★ 둘 중 하나라도 없으면 None — 0으로 채우면 결측이 '매력 없음'으로 바뀐다.
    """
