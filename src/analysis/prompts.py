...
ANALYSIS_SCHEMA: dict = {
    ...
    "$defs": {
        "trigger_list": {
            ...
            #   그래서 하한만 두고 **상한은 두지 않는다** — 상한이 있으면 모델이
            #   가장 뻔한 것부터 채우고 멈춘다.
            # ★ 하한을 3 이상으로 올리지 마라. 트리거가 정말 두 개뿐인 종목에서
            #   모델이 **빈칸을 지어내 채운다** — 검증 불가능한 트리거는 없느니만 못하다.
            # ★ 이 값은 **모델에게 주는 힌트일 뿐 강제가 아니다.** 저장 전 검증
            #   (`analyze.validate_payload`)은 손으로 짠 것이라 "비어 있지 않은가"만 본다.
            "minItems": 2,
            "items": {
                "type": "object",
                "properties": {
