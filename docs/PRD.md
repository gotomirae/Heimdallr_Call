...
- 열린 대시보드는 `constants.py`의 화면 갱신 간격에 따라 서버 데이터를 다시 읽는다.
  숨긴 탭은 쉬고 돌아오거나 네트워크가 복구되면 즉시 확인한다. DB 조회는 no-store를 유지한다.
- 발굴 목록의 기본 정렬은 최신 매크로에 따른 섹터 기회 → 투자 매력도 → 낮은 PRI → 낮은 내년 F.PER → 높은 내년 F.ROE → 영업이익 YoY → 최종 등급 → 최신 분기다. 머리글로 다른 열을 정렬하면 새 열이 1순위가 되되 기존 열의 오름·내림 상태는 동률 해소용으로 보존한다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
  ★/○ 비율·미반영 비율·점수·이익 양(+) 비율을 사전식으로 비교하며 하나의 점수로 합치지 않는다.
  한국은행 통화신용정책·경제전망과 KDI 경제동향의 최신 공식 원문·링크·현재/전망/전략을 함께 표시하되 뉴스 빈도는 점수화하지 않는다.
