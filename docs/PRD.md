...
| SC5 | 월 LLM 비용 | `cost_log` where env='prod' | $27 하드실링 이내, 동일 근거 재호출 0건 |
...
> **스코어에 합산하지 않는다.** 기업의 펀더멘털 매력과 주가 반영도는 서로 다른 축이고, 하나의
> 숫자로 뭉개면 둘 다 못 읽는다. 반드시 **2축으로 병기**한다.
...
### 4.1 성장 가속 판정 (전부 AND — 하나라도 실패하면 알림 대상 아님)

**사용자 화면 정의: 성장 가속 = 매출 YoY 가속 + 영업이익 YoY 가속 + OPM YoY 상승.**
G0·G3는 데이터 신뢰성과 유니버스 적합성을 보장하는 운영 조건이며 성장의 뜻에는 포함하지 않는다.
흑자전환은 YoY 비율을 만들 수 없으므로 `턴어라운드`로 별도 분류한다.
...
#### 4.2-1 투자 매력도 (기업·산업 대표 스코어, 100점)

발굴 목록의 대표 점수는 실적 가속만 나타내는 원점수가 아니라, 산업·기업·가치를 함께 본
**투자 매력도(기업·산업)**다. 현재 가격은 이 점수에 넣지 않고 PRI와 최종 등급에서 별도로 반영한다(ADR 5).
실적 원점수 A~D는 감사·근거용으로 보존하고, 대표 점수는 아래 7축을 측정 가능한 축만 분모에 넣어 계산한다.

| 축 | 배점 | 반영 내용 |
|---|---:|---|
| 산업 성장 | 10 | 같은 분기의 산업별 매출·영업이익 성장 순위 |
| 산업 내 위치 | 10 | 산업 내 매출·이익 성장률·OPM 상대 위치 |
| 실적 | 25 | 기존 A~D 실적 원점수 정규화 |
| 성장 스토리 | 20 | 전망 성장률·TTM 이익 개선·연속 가속 근거 |
| 밸류에이션 | 20 | 동일 산업 내 PER·F.PER 상대 위치(낮을수록 가점) |
| ROE | 8 | 올해·내년 예상 ROE의 동일 산업 내 상대 위치 |
| FCF | 7 | 최근 분기 FCF의 시총 대비 수익률·영업이익 현금전환 |

측정 불가능한 축은 0점이 아니라 분모에서 제외한다. 측정 배점 합계가 60점 미만이면 대표 점수를
판정하지 않는다. FCF는 정밀 수집 범위가 분기마다 다르므로 화면에 **최근 분기 FCF**라고 명시하며,
4분기 누적이라고 추정하지 않는다. 성장 스토리는 LLM 문장이 아니라 구조화된 수치 근거로 계산한다.
이 점수만으로 최종 등급을 정하지 않는다. §4.4에서 PRI와 교차해 현재 가격까지 반영한다.
...
| ID | 항목 | 배점 | 산식 |
|---|---|---|---|
| P1 | 실적 발표~현재 주가 반응 | 20 | 실적 발표 당일 종가 대비 현재 상승·하락률. 양수일수록 발표 실적이 가격에 더 반영됨 |
| P2 | 밸류에이션 | 20 | 네이버 내년 F.PER의 자기 과거 3개년 평균 PER 대비 할증·할인 |
| P3 | PEG(주가 수익 성장 비율) | 20 | 선행 이익 성장 1%당 PER을 같은 투자섹터 피어 중앙값과 비교. 유효 피어가 없으면 결측 |
| P4 | 이익 전망 반영 | 20 | 3·6·12M 상대수익률과 이익전망 변화의 괴리 + 현재 배수를 정당화할 내재 성장률과 컨센서스 성장률의 갭 |
| P5 | 가격 모멘텀·과열 | 20 | MACD·Signal 교차를 일간 차트에 명시하고, 반영도 계산은 중기 상대수익률·RSI 45 방향·5일 수익률·52주 고점 접근의 확인 가능한 값만 사용 |

> **낮을수록 아직 덜 반영됨.** 공개 원자료가 없는 항목은 0점으로 만들지 않고 각 핵심 요인 안에서 측정된 원자료만 정규화한다. 핵심 요인 세 개, 즉 측정 가능 배점 60점 미만이면 PRI 자체를 판정하지 않으며, 80점 미만은 숫자를 숨기지 않고 참고용 경고를 붙인다. 내재 성장률은 할인율·영구성장률을 추측한 DCF가 아니라 **자기 역사 배수로의 3년 정상화 역산**이다. PEG형 비교는 위험·자본효율을 제거하지 못하므로 시장 전체가 아닌 동일 투자섹터 안에서만 보조적으로 쓴다. 과열은 미래 매수자를 예언하지 않고 현재 가격의 단기 쏠림만 측정한다. 발표 직후 5거래일 외국인 수급은 시간축이 달라 이 축에 섞지 않는다.

### 4.4 2축 매트릭스 — 최종 분류 (알림·판단의 결론)

|  | **PRI < 40 (미반영)** | **PRI 40~65 (부분반영)** | **PRI > 65 (선반영/과열)** |
|---|---|---|---|
| **기업·산업 점수 ≥ 75** | ★ **최우선** | ○ **관심** | △ **추격 주의** |
| **기업·산업 점수 60~75** | ○ **관심** | · 관찰 | · 관찰 |
| **기업·산업 점수 < 60** | · 관찰 | · 관찰 | ✕ 제외 |

기업의 질과 현재 가격을 각각 보존한다. 최종 등급은 두 축을 교차해 현재 기준 투자 매력도를
표현하며, PRI를 기업 점수에 산술 합산하지 않는다.
...
- **자동 LLM 분석 대상: 성장 가속 중 알림 등급 ★·○만.** 성장 가속 종목·산업 전수는
  결정론적 스크리너와 대시보드가 계속 보여 주며, △··에는 매일 LLM 비용을 쓰지 않는다.

> **△(기업 고점수·선반영) 종목을 버리지 않는 이유**: 좋은 기업이지만 가격이 앞선 종목은
> **조정 시 담을 후보**다. 기업 점수가 유지된 채 PRI가 떨어지면 자동으로 ○/★로 승격된다.
...
| 분기 갱신 | **13콜/분기** | |
| 정밀 재무 (게이트 통과분) | 종목당 **최대 6콜** | 현재·직전·전년동기·전년연간 전체재무 + 현재·전년동기 주식총수. 이미 측정된 종목은 건너뜀 |
...
CREATE TABLE consensus_snapshots (
  ...
  per NUMERIC, fwd_per NUMERIC,              -- 네이버 최근 확정 / 올해 (E) PER
...
자동 유료 대상은 **성장 가속 종목 중 투자 알림 등급 ★·○만**이다. 한 분기 분석은
다음 세 번을 최대치로 삼되, 해당 이벤트와 실제 정보 변화가 있을 때만 호출한다(ADR 11).

1. `(1단계) 잠정실적 발표` — 잠정실적이 수집되자마자 초기 분석
2. `(2단계) 정기보고서 공시` — 같은 분기의 분기·반기·사업보고서 확정 재무와 발췌 반영
3. `(3단계) 리포트 창 종료` — 2단계 공시 뒤 **KOSPI 실제 5거래일**이 지난 시점에,
   최근 10달력일 내 증권사 리포트를 공개 웹검색으로 확인해 한 번 최종 분석

종목 상세의 **LLM 분석 버튼**은 자동 대상 밖의 종목도 현재 평가 분기 기준으로 단건 요청할
수 있다. Vercel은 장시간 모델 호출을 직접 실행하지 않고 private Supabase 큐에 접수하며,
5분 예약 worker가 비용 가드 뒤에서 처리한다. 최근 24시간 접수 상한과 종목·분기별 7일
재요청 간격을 적용한다. 일·월 사용량 소진 시 `deferred`로 보존하고 갱신 뒤 자동 재개한다.
즉시 분석은 Anthropic strict grammar의 내부 크기 제한을 피하도록 검색 전용 선택 필드를 뺀
필수 Canonical 계약을 사용한다. 생성 뒤에는 자동 배치와 같은 전체 스키마 검증을 통과한 결과만
저장하며, 실패 응답은 비용 감사 메타와 함께 `invalid`로 보존하되 투자 분석 화면에서는 숨긴다.

3단계 검색은 `선진짱 주식공부방`(`t.me/s/sunstudy1234`)과 `소중한추억.`
(`t.me/s/DOC_POOL`)을 먼저 확인하고, 부족할 때만 일반 웹검색으로 보완한다. 발행 증권사·제목·
발행일·실제 검색 결과 URL을 확인한 항목만 저장한다. 텔레그램은 발견 경로이며 연결된 증권사
원문과 공시를 사실 판단에서 우선한다. 컨센서스 변화가 없어도 3단계는 한 번 수행하되, 시세·
시가총액·PRI 변화만으로는 유료 분석을 다시 열지 않는다(ADR 13).
...
[유저 메시지] ~4,000토큰
 ...
 4-1) 3단계 최종 정기보고서 공시일 기준 snapshot과 실제 5번째 KOSPI 거래일 snapshot의
                 연간·향후 분기 컨센서스 변경값 + 공시 뒤 최근 10달력일 증권사 리포트 웹검색.
                 지정 텔레그램 2곳 우선, 발행일·직접 URL을 검증한 항목만 사용한다.
  5) 주가        현재가·실적발표 반응·전망 괴리·12M 이익/멀티플 분해·내재 성장률 갭·
                 자기 역사/피어 성장단가·중기 상대수익·단기 과열·PRI 분해
                 + 네이버 기업실적분석의 올해 연간 (E) PER·내년도 연간 (E) PER
 ...
 7) 업종 비교   동일 KRX 업종 상위 5개사의 시총·최신 평가 분기 매출·영업이익·OPM,
                 네이버/FnGuide 올해 예상 PER·ROE와 내년도 예상 PER·ROE
```

3단계에는 직전 4개 분기 정기보고서의 최신 정정본에서 제한 발췌와 접수번호 기반 DART
직접 링크를 추가한다. 이 1차 출처의 당시 주장과 이번 분기의 매출·영업이익·OPM을 대조하며,
숫자로 가시화되지 않으면 `판정불가`로 남긴다. 지정 텔레그램 2곳과 공개 웹검색은 최신
리포트·인터뷰 발견에 쓰고, 모든 리포트와 내러티브 판단에는 화면에 보이는 출처를 남긴다.
...
명시적으로 활성화한다. 운영 응답의 사실 필드에 입력으로 검증되지 않는 숫자가 있으면 그 숫자
토큰만 `[검증 불가 수치 삭제]`로 치환하고 정성 해석은 보존한다. 삭제 목록은 `_heimdallr` 메타와
화면에 남긴다. 같은 응답을 재결제해도 숫자 순응이 개선된다는 보장이 없기 때문이다(ADR 12).
...
```jsonc
{
  ...
  "broker_reports": [
    {"publisher": "증권사", "title": "리포트명", "published_at": "YYYY-MM-DD",
     "channel": "발견 경로", "url": "실제 검색 결과 URL", "key_point": "분석을 바꾼 논점"}
  ],
  "narrative_verification": [
    {"claim": "지난 4개 분기 당시 경영진 주장", "source_title": "원문", "source_date": "YYYY-MM-DD",
     "url": "검증된 직접 URL", "verdict": "실현|진행중|미실현|판정불가", "evidence": "이번 분기 실적 대조"}
  ],
...
- 성공 응답의 미근거 사실 숫자는 토큰만 제거하고 정성 해석을 보존한다. 구조·잘림 실패는
  `(단계, 근거 hash)`를 기록하며 같은 근거로 자동 재호출하지 않는다. 새 잠정·확정·컨센서스
  근거가 들어오면 hash 또는 단계가 달라져 다시 열리고, 수동 복구는 별도로 실행할 수 있다.
...
텔레그램 `web_app` 메뉴는 작은 내부 창을 강제하므로 사용하지 않는다. 일반 HTTPS 대시보드 버튼을 개인 채팅 상단 고정 메시지로 만들고, 메뉴 버튼은 기본형으로 되돌린다. 수신기 시작 때 현재 고정 메시지를 확인해 같은 URL이면 재발송하지 않는다.
...
### 8.6 📈 5·20일선 + MACD 상향 접근 알림 (평일 17:30 KST)

LLM 없이 결정론적으로 다음 조건을 모두 만족한 종목만 **매수 관찰** 알림으로 보낸다.

1. 출처·기준일이 명시된 세부산업의 향후 3년 CAGR이 **15% 이상**이고, 같은 투자 섹터의 매출·영업이익 YoY 중앙값도 2개 분기 연속 양수·상승하며 분기당 표본이 5종목 이상이어야 한다. CAGR 미측정 산업은 0%로 채우지 않고 제외한다.
2. 기업의 매출·영업이익 YoY가 각각 최근 2개 분기 연속 상승하고 현재 영업이익이 양수이며, 최근 분기 OPM이 직전 분기보다 상승해야 한다. 부호 전환 구간은 영업이익 YoY를 만들 수 없으므로 초기 흑전을 이 조건의 우회로로 쓰지 않는다.
3. 최신 평가 분기 최초 실적 공시일을 확인할 수 있어야 하며, 현재가가 최근 52주 또는 공시 이후 고점 대비 **20% 이상 조정**된 뒤 반등하거나 10거래일 좁은 범위에서 횡보해야 한다.
4. 일간 5일선이 상승하며 20일선 아래 2% 이내에서 간격이 좁아지거나 당일 상향 교차한다. **필수 조건**이다.
5. 일간 MACD(12·26·9)가 Signal 아래에서 반등하며 `MACD−Signal`이 종가 대비 -0.30~0%로 좁아지거나 당일 상향 교차한다. **필수 조건**이다.
6. 네이버 투자자별 매매동향에서 동일 주체인 **외국인 또는 기관**이 최근 3거래일 모두 순매수해야 한다. 외국인·기관이 날짜별로 번갈아 매수한 경우는 연속 수급으로 보지 않는다.
7. RSI(14)가 45 미만이고 직전 거래일 및 5거래일 전보다 상승했으면 **강력 신호 보강**으로 표시한다. RSI는 발송 필수가 아니며 일봉 기준일이 KST 오늘보다 5일 초과 오래됐으면 제외한다.

알림은 이모지·소제목·줄바꿈으로 산업의 올해/내년 공개 성장률·3년 CAGR·성장 스토리·이벤트와 출처, 기업의 2분기 성장 가속·OPM·수급, 현재가·5/20일선·MACD/Signal·RSI, 네이버 다음 분기 매출·영업이익 전망, 6개월 트리거를 구분한다. 대시보드 해당 종목·네이버증권·Heimdallr Call 링크를 모두 제공한다. 공개되지 않은 연도별 성장률·수요 원인·수주·고객사는 추정하지 않는다.
교차 직전과 당일을 구분하고 매수 확정 신호가 아님을 명시한다.
같은 종목·평가 분기·`technical_setup`은 한 번만 보내 반복 알림을 막는다.
하루 발송 상한은 재실행을 포함한 KST 하루 합계 2건이며, 조건별 탈락·조회 실패를 기록한다. 일일 요약 텔레그램에도 가격→5·20일선→MACD→3일 수급 통과, RSI 보강 및 발송 건수(실패 시 오류)를 표시한다. 엄격한 AND 조건으로 후보가 0건인 날은 정상 완료이며 연결 장애로 오인하지 않는다.
...
Kairos는 Google Drive 자료, 공시·IR, 증권사 기업/산업 리포트, 웹·지정 Telegram 채널을 조사하고 지정 Notion 양식에 작성한다. 첫 화면에는 짧은 투자 판단과 핵심 수치 표를 둔다. 본문은 장문 서술 대신 결론·수치·성장 영향·다음 확인을 표와 짧은 항목으로 제시한다. 중요한 판단·위험은 의미에 맞는 색상·굵기·밑줄로 강조한다. 분석일 기준 최근 3개월 내 성장에 큰 영향을 준 뉴스·공시·이벤트는 날짜, 확정/전망, 영향, 원문 링크를 표시한다. 분기 실적은 수치 표와 그래프를 함께 제공하고 공개된 수주·수출·제품·현금흐름 등 투자 판단에 중요한 비교 가능 숫자도 그래프로 보여 준다. 그래프에는 기간·단위·출처·해석을 붙인다. 원자료가 미공개인 수주 추이는 추정 그래프로 만들지 않는다. 네이버 증권·StockEasy 종목 차트 직접 링크와 공개 근거로 답한 주담 필수 질문 5개 이상을 포함한다. 실제 통화 기록이 없으면 공개자료 Q&A로 명시한다. 작업 중에는 Telegram 접수 메시지를 산업 조사·기업 검증·Notion 작성·저장 검증 단계로 갱신한다. 저장 위치·양식·출처·완료 상태를 재조회한 후에만 원래 개인 채팅으로 Notion 링크와 바로 열기 버튼을 보낸다. PDF를 만들지 않는다. 작업은 `pending → working → sending → sent`로 추적하며 중복 update ID는 재처리하지 않는다. 전송 결과가 불확실하면 `uncertain`으로 두고 자동 재전송하지 않는다. `pending` 깨우기는 claim이 없으면 5분 후 다시 시도한다. 로컬 컴퓨터와 Codex 앱이 꺼져 있으면 요청은 DB의 `pending`으로 보존된다.

조사는 산업부터 시작하여 시장 정의·규모·성장률·기술·수급·경쟁 구조와 기업의 위치를 숫자·도식·표로 설명한 후 기업 실적·가격 판단으로 이어진다. 산업은 [지정 산업분석 폴더](https://drive.google.com/drive/folders/1JchyHt19WRQacnLIDO1daQd3HLvfb15s)에서 해당 산업을, 기업은 [지정 기업분석 폴더](https://drive.google.com/drive/folders/1vnIBkOaoPcL3pC1n-V-LNUNJJhIm6hyd)에서 해당 기업을 찾아 분석일 기준 최근 3개월 내 가장 최신 자료를 1차로 확인한다. 두 지정 Telegram 채널(DOC_POOL, sunstudy1234)은 매 요청에서 각각 접근·검색을 시도한다. 채널 접근이 막히면 재시도하거나 다른 로그인 경로를 찾지 않고 실패 범위를 기록한 뒤 증권사 공식 리서치센터 및 접근 가능한 공개 리포트 사이트, 공시·IR·산업 통계 등 다른 자료로 분석을 끝낸다. 채널 게시물·리포트 원문 열람 여부를 구분하고 중요 수치는 공식 원문으로 검증한다. 채널 접근 실패만으로 `working` 유지·Notion 게시 보류·Telegram 링크 발송 보류를 하지 않는다.

사용량 제한 중단 시 `working`과 요청 ID별 로컬 장부를 보존한다. `claim` 후 기존 Codex 작업의 반복 확인을 활성화하고, 미완료 `working`에 한해 사용량을 조회한다. 사용 가능량이 돌아오면 저장한 원고·출처·Notion 페이지 상태에서 재개한다. `working`이 없으면 확인을 일시 중지해 유휴 사용량을 쓰지 않는다. 페이지 생성과 Telegram 발송은 저장/전달 상태를 대조해 중복하지 않는다. 요청이 없으면 분석·Notion 작성·알림을 하지 않는다.

Notion 보고서의 표 머리글·지표·단위·그래프 제목·축·범례·설명은 한국어로 작성한다. 본문 시작 부분에 목차 블록을 넣지 않는다. 상세 분석을 유지하면서 문장 마침표 뒤에는 다음 문장을 새 줄·블록으로 시작하고, 표 셀의 두 문장은 `<br>`로 구분한다. 주가 절은 네이버 증권의 일봉 기준일·종가와 과거 상승·하락의 주요 구간을 수치·그래프·날짜별 확인 사건으로 설명하되, 사건과 가격 변화의 인과관계는 검증 범위에 맞춰 해석으로 표시한다. Heimdallr `/stock/{code}`·네이버 증권·StockEasy의 해당 종목 직접 링크를 제공한다. 저장 후 목차 생략·한글 표/그래프·이미지 표시·가격과 출처·세 직접 링크를 재조회한다.
...
| `/` | 성장 가속 / 매출 YoY 둔화+영익 YoY 가속 / 턴어라운드 / 기타 / 전 종목 분류, 반도체 4단계 밸류체인, 현재 투자 우선순위와 다중 정렬 | 2 |
| `/watchlist` | 발굴 목록에서 선택한 관심 종목. 브라우저에 영구 저장 | 3 |
| `/matrix` | **2축 산점도** (X=투자 매력도(기업·산업), Y=주가반영도). 사분면 색상 구분, 점 클릭 시 상세 | 3 |
| `/season` | 시즌 진행률 + 발표 캘린더 + 미발표 종목 | 5 |
| `/outcome` | **결과 추적** — 등급별 D+20/D+40/D+60 초과수익 분포, 축별 IC | 6 |
| `/settings` | 임계값 · 월별 LLM 비용 · 다음 달 예측 | 7 |

### 9.1 `/stock/[code]` 구성

1. **헤더** — 종목명·코드·업종·시총 / **한국시간 당일 16:00에 확정된 네이버 종가**·등락·거래일(16시 전에는 전 거래일, 실패 시 마지막 저장일 명시) / 52주 위치 게이지 / 3·6·12M 수익률 (지수 대비 병기)
2. **판정 카드** — 등급(★○△·) / **투자 매력도 7축 스택 바** / 실적 원점수 A·B·C·D 감사값 /
   **주가반영도 5개 핵심 요인 분해 바와 발표~현재 반응·3개년 평균 대비 F.PER·PEG·이익 전망·MACD/RSI 가격 과열 근거** / 분기 내 백분위 /
   경고 배지(기저효과·업종주의·컨센없음)
3. **분기 실적 추이 (10분기 + 다음 분기 컨센서스) + 실제 일간 종가** — 녹색 매출 막대와 굵은 흰색 GPM, 황금색 영업이익 막대와 흰색 OPM을 표시한 뒤 매출·영업이익 YoY → 수주잔고·신규수주 순서로 표시한다. GPM은 OpenDART 전체 재무제표의 매출총이익 누적치를 분기 단독치로 분해한 뒤 같은 분기 매출로 나눈 값만 쓰며, 원자료가 없으면 0% 선을 만들지 않고 측정 불가를 밝힌다. 다음 분기 네이버 컨센서스는 `(E)`와 점선·윤곽 막대로 구분하고 전년 동분기 실적과 대조해 YoY를 계산하며 성장률의 원자료·추정기관·스냅샷일을 붙인다. 발표 분기의 영업이익 성장률 판단과 다음 분기 전망은 별개 카드로 표시한다. 두 YoY는 실제 %를 한 좌표에 놓고 모든 분기의 두 원값을 바로 아래에 병기한다. 결측을 선으로 잇지 않으며 흑전·적전은 상태 라벨로 표시한다. 정기보고서 수주잔고·신규수주는 단위·분기·공시 범위를 검증한 경우만 표시한다. OpenDART `단일판매·공급계약체결`은 접수번호·공시일·계약명·계약금액·최근 매출 대비·상대방·기간을 별도 표로 구조화하고 해당 원문에 연결한다. 이 계약금액은 공시된 개별 계약일 뿐 전체 회사 신규수주가 아니므로 정기보고서 수주잔고와 나란히 대조하되 임의 합산하지 않는다. 기재 제한은 금액 0이 아니라 비공개 상태로 표시한다. DART에서 구조화하지 못한 계약·공급 이벤트는 LLM 웹검색에서 직접 URL이 확인된 최근 공개 원문을 날짜별 보완 목록으로 분리한다. 주가는 네이버 일봉을 그대로 쓰며 과거 각 분기의 잠정·확정 발표일을 점선과 라벨로 모두 표시한다. 종가·MACD·RSI는 같은 날짜축 `syncId`로 연결해 한 그래프의 특정 시기를 가리키면 세 그래프의 커서가 함께 움직인다. MACD·Signal의 의미와 현재 교차 방향, RSI(14)의 의미와 45선 추세를 실제 최신값으로 설명한다. 주가 원인은 수치 변화와 매크로·기업 펀더멘털 사건을 출처 기반 스토리로 함께 보인다.
4. **분기 히스토리 표** — 최근 10개 분기를 누락 없이 분기 · 매출/YoY/QoQ · 영업이익/YoY/QoQ · OPM · FCF · 잠정/확정 구분으로 표시
5. **밸류에이션** — 네이버 기업실적분석의 **올해 PER(예상)·내년 F.PER(예상)·올해/내년 ROE·FCF**를 계산 없이 같은 기준일·연도축으로 표시한다. **PEG만** 네이버 내년 F.PER ÷ 네이버 올해→내년 EPS 성장률(%)로 자체 계산한다. 네이버 최근 확정 3개년 PER 평균과 내년 F.PER를 명시적으로 비교하고, 네이버에 값이 없으면 다른 원천으로 채우지 않는다. 별도 `컨센서스 대비` 상세 카드는 표시하지 않는다.
6. **LLM 분석** — 자동 분석은 성장 가속 ★·○에 적용하고, 모든 종목은 버튼으로 단건 요청할 수 있다. `(1단계) 잠정실적` → `(2단계) 정기보고서` →
   `(3단계) LLM 추가 분석` 배지 / 분석 기준일·진행 상태 / 리포트 발행일·직접 링크·핵심 논점 /
   핵심 내용 / 주요 고객과 글로벌 상·하류 밸류체인 / 기준일 직전 3개월 글로벌 기업향 확인 수주·협업(날짜·원문, 없으면 확인 범위) / 트리거 / 시나리오 / 리스크 / 가치-가격 비교 / 지난 4개 분기 경영진 발언·실적발표 주장과 이번 분기 숫자의 출처 링크 기반 내러티브 검증.
   - 버튼 요청은 `접수·대기 → 입력/원문/LLM 분석·검증 → 저장 완료`의 **단계 기준 진행률**, 접수 시각, worker 처리 시작 시각을 10초 간격으로 갱신한다. 모델 내부 토큰 생성률로 오인하지 않도록 화면에 구분하고, 페이지를 다시 열어도 활성 요청을 자동 추적한다.
   - **판매 제품·사업 매출 비중**은 평가 분기의 1분기보고서·반기보고서·3분기보고서·사업보고서와 누적 기간, DART 접수번호를 함께 표시한다. 값 셀의 `%`뿐 아니라 표 머리글의 `비중(%)` 계약도 읽되 금액으로 역산하지 않는다. 수집 대기·절 미포함·발췌 잘림·비중 열 미기재를 구분한다.
   - **수주 확인 포인트** — 평가 분기와 같은 DART 발췌에 `수주잔고`·`수주총액`·`신규 수주`가 실제로 있을 때만 원문 문맥을 별도 표시한다. 한 분기뿐이므로 **QoQ·증가 판정을 만들지 않으며**, 비공개와 발췌 잘림을 배지로 밝힌다. 새 원문 파서 적용 여부를 발췌 안에 기록하여 기존 짧은 발췌도 예약 수집기가 한 번씩 재검사하고, 완료된 접수번호는 반복 호출하지 않는다.
   - 전 종목 최신 정기보고서는 별도 점진 수집 작업으로 채운다. 수치가 없더라도 `비공개·기재 생략`, `해당 없음`, `수주 언급·단일 수치 미확인`, `원문 수집 대기`를 구분해 화면에서 숨기지 않는다.
7. **업종 비교 표** — 같은 섹터이면서 주요 제품 설명에 의미 있는 공통어가 확인된 기업만 비교한다. 시총·매출·영업이익·OPM·올해/내년 ROE·올해/내년 예상 PER와 표시 종목 중앙값을 보이며 자체 PER을 계산해 채우지 않는다. 가로 스크롤에서도 종목명 열은 고정한다.
8. **결과 추적** — 발표 후 D+1/5/20/**40**/60 수익률 (지수 대비)
9. **원문 링크** — 네이버 증권 · StockEasy 종목 상세 · 접수번호 기반 DART 공시 원문. 내부 LLM 발췌 전문은 화면에 중복 노출하지 않는다.
...
| `dashboard_analysis.yml` | `*/5 * * * *` | 종목 상세 버튼 요청을 최대 3건씩 처리. 비용 한도 소진 요청은 보존 후 자동 재개 |
| `order_disclosure.yml` | `37 17 * * 1-5` (평일 02:37 KST) | 전 종목 최신 정기보고서 수주잔고·신규수주 원문을 30분 시간 예산으로 점진 수집 |
| `daily_digest.yml` | `37 8, 17 9, 7 10 * * 1-5` (17:37·18:17·19:07 KST) | 조건 충족 종목을 하루 최대 2개 추천하고 일일 요약 발송. 예약 지연에 대비해 3회 실행하며 KST 일일 상한·종목/분기 중복키로 재발송 차단 |
| `quarterly_backfill.yml` | `0 20 1,15 * *` + 수동 | 최근 45일 미반영 정기보고서 증분 백필 → 1차 게이트 → 정밀 재무(게이트 통과분) → D축 포함 `score_final` 재계산. 전체 연도는 수동 복구 옵션 |
| `outcome_update.yml` | `0 22 * * 1-5` (07:00 KST) | D+1/5/20/40/60 수익률 갱신 |
| `promotion_check.yml` | `0 22 * * 1` (월요일) | △→○/★ 승격 확인 및 알림 |

### 10.0 변경 자동 반영 (2026-09-05)

- 공시를 매일 주간 30분·야간 3시간 간격으로 확인한다. 계절 변수 수동 전환은 필요 없다.
  최근 7일을 재조회하며 성공한 잠정공시는 접수번호로 중복 처리를 막고, 확정치를 잠정치로 되돌리지 않는다.
- 같은 날의 정정공시도 감지 시각·접수번호로 구분한다. 같은 분기 원문은 최신 접수번호를 선택하고,
  정정에 따른 정밀 재무도 다시 수집한다. 누락된 신규 확정 종목의 정밀 재무는 폴링 및 매일 배치가 보충한다.
- 자동 LLM은 성장 가속 중 ★·○만 분석한다. 잠정→정기보고서→5거래일 뒤 최근 10일 리포트
  웹검색의 세 이벤트만 연다. 3단계는 컨센서스 변화가 없어도 딱 한 번 수행하며, 매일 변하는
  시세·시가총액·PRI에는 호출하지 않는다.
  레거시 메타 보강이나 수집 시각만으로 과거 분석을 다시 열지 않는다. 실패한 `(단계, 근거 hash)`도
  자동 재시도하지 않는다. 월·일 비용 제한은 최후 방어선으로 유지한다.
- 열린 대시보드는 `constants.py`의 화면 갱신 간격에 따라 서버 데이터를 다시 읽되, 클릭·입력·스크롤
  직후에는 전체 서버 갱신을 보류한다. 숨긴 탭에서 돌아오거나 네트워크가 복구되면 즉시 확인하고,
  일시적 연결 실패에는 현재 화면 상태를 보존한 재시도 UI를 보이며, 재시도 버튼은 오류 경계만
  초기화하지 않고 전체 문서를 다시 요청해 서버 DB 연결까지 새로 만든다. DB 조회는 no-store를 유지하며,
  연결·pool·timeout 오류만 짧게 재시도하고 영구적인 인증·스키마 오류는 그대로 올린다. 시세·컨센서스·
  발표 후 수익률·분기 실적 중 하나가 끝까지 실패해도 핵심 발굴 목록은 유지하고 해당 값만 미수집으로
  밝힌다. 표는 전수 데이터를 정렬·필터에 보존하되 초기 120행부터 120행씩 점진 렌더한다.
  서버→브라우저 전송은 고정 순서 tuple 계약으로 반복 필드명을 제거하고 브라우저에서 원래 행으로
  복원한다. 종목 수·필터·정렬·결측 의미는 압축 전과 같아야 한다. 상단 탭은 브라우저 유휴 시간과 포인터 진입 때 RSC 경로를 사전 로드해 전환 대기를 줄이되, 서버 데이터의 `no-store` 계약은 유지한다.
- 발굴 목록의 기본 정렬은 미국 중심 글로벌 매크로에 맞는 섹터 기회 → 투자 매력도 → 낮은 주가반영도 → 낮은 내년 F.PER → 높은 내년 F.ROE → 영업이익 YoY → 최종 등급 → 최신 분기다. 먼저 누른 머리글의 우선순위·방향을 고정하고 새 열을 다음 동률 해소 규칙으로 붙인다. 각 열은 기본 → 오름차순 → 내림차순 → 기본 순서로 순환하며, 기본 복귀 시 그 열만 제거하고 다른 열의 우선순위·방향은 보존한다. 개별 ×와 전체 기본 추천 순서 복원을 제공한다. 표 바로 위에는 종목명/코드·성장 유형·등급·섹터·시총·영업이익 컨센 상회/하회·분기 필터를 둔다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
  ★/○ 비율·미반영 비율·점수·이익 양(+) 비율을 사전식으로 비교하며 하나의 점수로 합치지 않는다.
  미국·글로벌 영역은 시장 온도·전망 변수·자동 계산 흐름·공식 발표 요약표·적합 섹터·초기 성장 기준 순으로 시각 구분한다. `추천 정렬`은 이 영역 안에서 반복하지 않고 종목 목록 표 바로 위에만 큰 황금색 굵은 글씨로 표시한다.
  매일 06:45 KST 사전 예열 후 07:00 KST 갱신 목표(07:15·07:30·08:00·08:30·09:00 재시도)의 별도 배치에서 미국 마지막 **완료된** 거래일의 S&P 500·나스닥·다우·필라델피아 반도체·CBOE VIX 60거래일과 Fear & Greed 60일, 미 연준의 최신 FOMC 성명을 수집·검증해 스냅샷을 갱신한다. VIX·심리 지수는 임계선과 간단한 의미를 표시한다. Fear & Greed와 나스닥 종가는 같은 날짜의 이중축 그래프에 겹치고, 수준이 아니라 일간 변화의 Pearson 상관계수와 인과관계가 아니라는 설명을 함께 표시한다. TradingView S&P 500 히트맵으로 섹터 확산을 함께 본다. 현재일부터 92일 안의 BLS·BEA·FOMC 공식 일정을 날짜·확인 변수·대응 방안으로 표시하며, 중요 일정은 최대 3개의 황금 별표로 제한한다. 사전 예열이 성공해도 07시 이후 첫 실행에서 기준 시각을 다시 기록한다. 미국 AI·데이터센터
  투자와 연결된 반도체·전력 공급망은 시장 국면에 따라 우선순위를 조정하되 구조화 실적에서 ★/○가 확인된 섹터만 승격한다. 종가 출처는 Yahoo Finance와 TradingView, 통화정책은 미 연준, 물가·고용 참고 링크는 BLS로 구분하고 시장 기준일·갱신 시각을 표시한다. 일정 제목은 BLS·BEA·연준의 공식 문서를 새 탭으로 직접 열고, 옆의 **한국어 핵심 설명**은 외부 번역 프록시에 의존하지 않는 대시보드 내부 페이지로 연다.
  뉴스 빈도는 점수화하지 않는다. 외부 사이트 호출은 페이지 렌더 경로에서 분리한다. GitHub 예약 실행·배포 지연 때문에 정확히 07:00 표시를 보장할 수 없으며, 갱신 실패 시 이전 날짜를 숨기지 않고 최근 완료 미국 거래일과 갱신일을 함께 검사해 경고한다.
- 결과 추적의 다음 분기 전망은 네이버 종목별 다음 분기 매출·영업이익 컨센서스를 전년 동분기와 이번 분기 실적에 대조해 섹터 중앙값·QoQ·가속 예상 비율로 표시한다. 영업이익 부호 전환은 성장률로 만들지 않고, 섹터 표본 5개 미만 또는 컨센서스 공백일 때만 직전 추세의 조건부 범위를 보인다.
- 투자 섹터는 매일 유니버스 갱신 직후, 스크리닝 전에 저장한다. 반도체는 정확히
  `반도체 IDM / 반도체 DSP / 반도체 OSAT / 반도체 소재 / 반도체 부품 / 반도체 장비`
  여섯 밸류체인으로 나눈다. 소재·부품·장비는 주요제품 증거로 전공정 `전` 또는
  후공정 `후`를 작은 표식으로 병기하며, OSAT는 정의상 `후`로 표시한다.
  IDM은 양 공정을 함께 할 수 있고 DSP는 설계 단계이므로 공정 표식을 추정하지 않는다.
  저장된 과거 섹터가 전자부품이어도 주요 제품이 반도체 제품임을 명시하면 반도체 부품을 우선한다. 전자부품은 반도체와 무관한 제품이 본업일 때만 유지한다.
- 발굴 목록에서는 기존 섹터를 선택하고 새 이름을 추가할 수 있으며 종목별 사용자 수정을 브라우저에 저장한다. 이 수정은 즉시 필터·정렬·표시에 반영하되 인증 없는 공개 화면에서 서버 공통 분류를 바꾸지 않는다. 운영 공통 분류 변경은 제품 근거를 검증한 수집·분류 배치로 수행한다.
- 결과 추적은 100행마다 저장한다. 발표 당시 판정을 보존하고, 일시적인 조회 실패로 기존 수익률을 지우지 않는다.
- GitHub Actions는 예약 시각보다 늦게 시작될 수 있다. 공시 원천 공개 → 다음 수집 실행 → DB 저장 → 화면 갱신 순서이며,
  변경 발생 순간의 실시간 푸시를 보장하지 않는다. 시세·컨센서스는 매일 06:00 KST 예약 배치에서 갱신한다.
...
| 항목 | 단가 | 수량 | 분기 비용 |
|---|---|---|---|
| LLM 분석 (Sonnet 5) | 실측 단가를 `cost_log`로 추적 | ★·○의 실제 정보변화 이벤트만 | 월 $27 하드실링 |
| 3단계 서버 웹검색 | $0.01/회 | 종목당 최대 3회, 3단계에서만 | 실제 usage를 위 실링에 합산 |
| 잠정실적 Haiku 폴백 | $0.012/건 | 60건 | $0.72 |
| 일일 요약 생성 | $0.01/일 | 30일 | $0.30 |
| **분기 합계** | | | `잠정 대상 + 정기보고서 대상 + 최근 리포트 최종갱신 대상`의 실호출 합계 |
...
**가드레일**: 월 하드실링과 일일 배치 상한은 `src/config/constants.py`가 단일 출처다.
하드실링은 사고의 최대 손실을 제한할 뿐 목표 지출이 아니다. 정상 운영 비용은 동일 근거
재시도 금지, ★·○ 한정, 3단계 종목당 1회·검색 최대 3회 제한으로 먼저 줄인다.
...
# PRI 4.0 — 주가 영향 핵심 5개 요인
PRI_NEW_WEIGHTS = {"earnings_reaction": 20, "expectation_gap": 20,
                   "earnings_vs_multiple": 20, "valuation_burden": 20,
...
| 구분 | 주의 사항 |
|---|---|
| 건설 · 조선 · 플랜트 | 진행기준 매출이라 분기 변동이 크다. 같은 분기 수주 원문은 확인 포인트로 표시하되, 수주잔고 QoQ는 비교 분기 원문이 없어 만들지 않는다. |
