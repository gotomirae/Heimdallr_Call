...
| **G2** | `op(t) > 0` **AND** `op_yoy(t) > 0` | 탈락 (단, 흑자전환은 `turnaround=true`로 별도 리스트) |
...
| ID | 항목 | 배점 | 산식 |
|---|---|---|---|
| A1 | 매출 YoY 델타 | 14 | `Δ = rev_yoy(t) − rev_yoy(t−1)` · Δ≤0→0 · Δ≥20%p→14 · 선형 |
| A2 | 영업이익 YoY 델타 | 10 | 동일 방식 · Δ≥40%p→10 |
| A3 | **TTM 매출 추세** | 6 | `TTM_rev(t) > TTM_rev(t−1)`→3 · 증가율 ≥5%→+3 |
| A4 | 연속 가속 | 5 | 2분기 연속 G1 충족→5 |
...
```
[시스템 프롬프트 — cache_control: ephemeral] ~2,500토큰
