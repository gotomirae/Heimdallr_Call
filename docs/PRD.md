...
| ID | 항목 | 배점 | 산식 |
|---|---|---|---|
| P1 | 실적 발표 반응 | 20 | 발표 후 D+1·5·20 지수 대비 평균. 실적 공개 직후 시장보다 먼저 움직인 정도 |
| P2 | 이익 전망 반영 | 20 | 3·6·12M 상대수익률과 90일 이익전망 변화의 괴리 + 현재 배수를 정당화할 내재 성장률과 컨센서스 성장률의 갭 |
| P3 | 이익 성장 대 멀티플 | 20 | `(1+주가수익)=(1+TTM 이익성장)×(1+PER변화)`로 12M 상승을 분해한 멀티플 기여도. 주가가 하락했으면 상승 원인을 만들지 않고 제외 |
| P4 | 밸류에이션 부담 | 20 | 현재 TTM PER의 자기 9분기 평균 대비 할증 + 동일 투자섹터 피어 성장조정 PER 대비 할증. 유효 피어 5개 미만이면 자기 역사만 정규화 |
| P5 | 가격 모멘텀·과열 | 20 | 3·6·12M 지수 대비 초과수익 + 현재 RSI·5일 수익률·52주 고점 접근 합성. 단기 과열 신호는 2개 미만이면 제외 |
...
2. **판정 카드** — 등급(★○△·) / **투자 매력도 7축 스택 바** / 실적 원점수 A·B·C·D 감사값 /
   **PRI 5개 핵심 요인 분해 바와 실적 반응·이익 전망·이익/멀티플·밸류 부담·가격 과열 근거** / 분기 내 백분위 /
   경고 배지(기저효과·업종주의·컨센없음)
3. **분기 실적 추이 (10분기 + 다음 분기 컨센서스) + 실제 일간 종가** — 매출액 → 영업이익·OPM → 매출·영업이익 YoY → 수주잔고·신규수주 순서로 표시한다. 다음 분기 네이버 컨센서스는 `(E)`와 점선·윤곽 막대로 구분하고 전년 동분기 실적과 대조해 YoY를 계산한다. 발표 분기의 영업이익 성장률 판단과 다음 분기 전망은 별개 카드로 표시한다. 두 YoY는 실제 %를 한 좌표에 놓고 모든 분기의 두 원값을 바로 아래에 병기한다. 결측을 선으로 잇지 않으며 흑전·적전은 상태 라벨로 표시한다. 수주잔고·신규수주는 DART 정기보고서의 단위·분기·공시 범위를 검증한 경우만 표시하고, 주요계약 합계를 전체 회사 잔고나 신규수주로 바꾸지 않는다. 주가는 네이버 일봉을 그대로 쓰며 분기실적 발표일을 세로선으로 표시하고, MACD(12·26·9)·RSI(14)도 거래일 기준으로 계산한다. 각 그래프 바로 아래에 현재 위치가 나온 이유·확인 지표·매수/매도 활용법을 결정론적으로 설명하고, 주가 원인은 LLM의 출처 기반 가격 분석을 함께 보인다.
4. **분기 히스토리 표** — 최근 10개 분기를 누락 없이 분기 · 매출/YoY/QoQ · 영업이익/YoY/QoQ · OPM · FCF · 잠정/확정 구분으로 표시
5. **컨센서스 대비** — 서프라이즈 % + 추정기관 수 (없으면 "커버리지 없음" 명시)
6. **밸류에이션** — 네이버 기업실적분석의 **올해 PER(예상)·내년 F.PER(예상)·올해 ROE(예상)·내년 F.ROE**를 같은 기준일·연도축으로 표시한다. 네이버에 다음 연도 값이 없으면 다른 원천으로 채우지 않는다. 업종 중앙값 비교 / PBR·PEG / 3년 밴드 내 위치 / 주가 위치 판정
...
8. **업종 비교 표** — 같은 평가 분기 상위 5개사의 시총·매출·영업이익·OPM·올해/내년 ROE·올해/내년 예상 PER와 표시 종목 중앙값
...
- 발굴 목록의 기본 정렬은 미국 중심 글로벌 매크로에 맞는 섹터 기회 → 투자 매력도 → 낮은 PRI → 낮은 내년 F.PER → 높은 내년 F.ROE → 영업이익 YoY → 최종 등급 → 최신 분기다. 먼저 누른 머리글의 우선순위·방향을 고정하고 새 열을 다음 동률 해소 규칙으로 붙인다. 각 열은 기본 → 오름차순 → 내림차순 → 기본 순서로 순환하며, 기본 복귀 시 그 열만 제거하고 다른 열의 우선순위·방향은 보존한다. 개별 ×와 전체 기본 복원도 제공한다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
  ...
  투자와 연결된 반도체·전력 공급망은 시장 국면에 따라 우선순위를 조정하되 구조화 실적에서 ★/○가 확인된 섹터만 승격한다. 종가 출처는 Yahoo Finance, 통화정책은 미 연준, 물가·고용 참고 링크는 BLS로 구분하고 시장 기준일·갱신 시각을 표시한다.
