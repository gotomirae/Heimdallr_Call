...
3. 최신 평가 분기 최초 실적 공시의 **다음 거래일 종가**보다 현재 종가가 낮거나, 공시 이후 고점 대비 10% 이상 조정한 뒤 20일 수익률이 음수인 하락 또는 10일 수익률 ±3%·고저폭 8% 이내 횡보. 발표일을 확인할 수 없으면 추정하지 않고 제외한다.
4. 일간 MACD(12·26·9)가 Signal 아래에 있고, `MACD−Signal`이 3거래일 연속 상승하면서
   종가 대비 절대 간격 0.30% 이내
5. RSI(14)가 40~50이며 최근 3거래일 연속 상승. 일봉 기준일이 KST 오늘보다 5일 초과 오래됐으면 제외한다.
...
하루 발송 상한은 재실행을 포함한 KST 하루 합계 2건이며, 조건별 탈락·조회 실패를 기록한다. 일일 요약 텔레그램에도 가격→MACD→RSI 통과 및 발송 건수(실패 시 오류)를 표시해 0건과 연결 장애를 구분한다. 조건을 통과한 후보 중 MACD 히스토그램의 종가 대비 절대 간격이
...
- 발굴 목록의 기본 정렬은 미국 중심 글로벌 매크로에 맞는 섹터 기회 → 투자 매력도 → 낮은 PRI → 낮은 내년 F.PER → 높은 내년 F.ROE → 영업이익 YoY → 최종 등급 → 최신 분기다. 먼저 누른 머리글의 우선순위·방향을 고정하고 새 열을 다음 동률 해소 규칙으로 붙인다. 각 열은 기본 → 오름차순 → 내림차순 → 기본 순서로 순환하며, 기본 복귀 시 그 열만 제거하고 다른 열의 우선순위·방향은 보존한다. 개별 ×와 전체 기본 복원도 제공한다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
  ...
  매일 07:40 KST 사전 예열, 08:00 KST 갱신 목표(08:30·09:00 재시도)의 별도 배치에서 미국 마지막 **완료된** 거래일의 S&P 500·나스닥·필라델피아 반도체·VIX와 미 연준의 최신 FOMC 성명을 수집·검증해 스냅샷을 갱신한다. 미국 AI·데이터센터
  ...
  뉴스 빈도는 점수화하지 않는다. 외부 사이트 호출은 페이지 렌더 경로에서 분리한다. GitHub 예약 실행·배포 지연 때문에 정확히 08:00 표시를 보장할 수 없으며, 갱신 실패 시 이전 날짜를 숨기지 않는다.
