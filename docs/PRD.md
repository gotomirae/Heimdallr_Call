...
1. 같은 투자 섹터의 매출·영업이익 YoY 중앙값이 2개 분기 연속 양수이고 각각 상승하며 분기당 표본이 5종목 이상. 이는 실현된 산업 성장의 보수적 전망 프록시이며 미래 컨센서스로 오인하지 않는다.
2. 기업의 매출·영업이익 YoY가 각각 2개 분기 연속 상승하며 현재 영업이익이 양수이거나, 최근 분기가 **초기 흑전**이고 매출 YoY가 직전 분기보다 높다. 초기 흑전·낮은 PRI·★/○를 우선 추천하지만 실제 수주 증가는 확인된 공시가 없으면 주장하지 않는다.
3. 최신 평가 분기 최초 실적 공시의 **다음 거래일 종가**보다 현재 종가가 낮거나, 공시 이후 고점 대비 10% 이상 조정했다. 하락·횡보뿐 아니라 아직 주가에 반영되지 않은 회복 구간도 허용한다. 발표일을 확인할 수 없으면 추정하지 않고 제외한다.
4. 일간 5일선이 상승하며 20일선 아래 2% 이내에서 간격이 좁아지거나 당일 상향 교차한다. **필수 조건**이다.
5. 일간 MACD(12·26·9)가 Signal 아래에서 올라오며 `MACD−Signal`이 종가 대비 -0.30~0%로 좁아지거나 당일 상향 교차한다. 3거래일 연속 상승은 요구하지 않는다. **필수 조건**이다.
6. RSI(14)가 45 미만이고 직전 거래일 및 5거래일 전보다 상승했으면 **강력 신호 보강**으로 표시한다. RSI는 발송 필수가 아니며 3거래일 연속 상승도 요구하지 않는다. 일봉 기준일이 KST 오늘보다 5일 초과 오래됐으면 제외한다.

알림에는 산업·기업 성장률 이력, 조정폭, MACD/Signal/Gap, RSI, 대시보드·네이버 링크를 표시한다. 투자 아이디어는 기술 지표를 반복하지 않고 주요 제품·매출과 영업이익 가속·OPM 변화·투자 매력도·PRI·공개된 선행 PER/ROE를 근거로 기업 펀더멘털 가설과 다음 분기 확인 항목을 제시한다. 공개되지 않은 수요 원인·수주·고객사는 추정하지 않는다.
