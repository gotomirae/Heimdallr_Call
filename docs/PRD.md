...
3. **분기 실적 추이 (10분기 + 다음 분기 컨센서스) + 실제 일간 종가** — 매출액 → 영업이익·OPM → 매출·영업이익 YoY → 수주잔고·신규수주 순서로 표시한다. 다음 분기 네이버 컨센서스는 `(E)`로 구분하고 전년 동분기 실적과 대조해 YoY를 계산한다. 두 YoY는 실제 %를 한 좌표에 놓고 모든 분기의 두 원값을 바로 아래에 병기한다. 결측을 선으로 잇지 않으며 흑전·적전은 상태 라벨로 표시한다. 주가는 네이버 일봉을 그대로 쓰며 분기실적 발표일을 세로선으로 표시하고, MACD(12·26·9)·RSI(14)도 거래일 기준으로 계산한다. 각 그래프 바로 아래에 현재 위치가 나온 이유·확인 지표·매수/매도 활용법을 결정론적으로 설명하고, 주가 원인은 LLM의 출처 기반 가격 분석을 함께 보인다.
...
7. **LLM 분석** — 성장 가속 종목에만 표시. `(1단계) 잠정실적` → `(2단계) 정기보고서` →
   ...
   핵심 내용 / 트리거 / 시나리오 / 리스크 / 가치-가격 비교 / 지난 4개 분기 경영진 발언·실적발표 주장과 이번 분기 숫자의 출처 링크 기반 내러티브 검증.
