...
- 발굴 목록의 기본 정렬은 미국 중심 글로벌 매크로에 맞는 섹터 기회 → 투자 매력도 → 낮은 PRI → 낮은 내년 F.PER → 높은 내년 F.ROE → 영업이익 YoY → 최종 등급 → 최신 분기다. 먼저 누른 머리글의 우선순위·방향을 고정하고 새 열을 다음 동률 해소 규칙으로 붙인다. 같은 열은 내림·오름만 전환하며 제거는 개별 × 또는 기본 복원으로 명시한다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
