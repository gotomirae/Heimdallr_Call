...
3. **분기 실적 추이 (9분기) + 실제 주간 종가** — 매출 → 매출 YoY → 영업이익 → 영업이익 YoY → OPM → 수주잔고 → 신규수주 순서를 유지한다. 매출 YoY·영업이익 YoY는 한 그래프에, 수주잔고·신규수주는 한 그래프에 비교 표시하며 모든 시계열 값은 각 분기 그래프 바로 위에 숫자로 표시한다. 수주 정형 수치가 없으면 미수집을 밝힌다. 주간 종가는 같은 기간의 종가·MACD(12·26·9)·RSI(14)를 별도 패널로 표시한다.
