...
6. **밸류에이션** — 네이버 현재 공개값의 최근 4분기 PER·F.PER·연간 (E) ROE를 같은 기준일로 표시하고 DART 최근 4개 분기 순이익·현재 시총 계산값은 검산으로만 병기한다. 네이버에 다음 연도 ROE가 없으면 다른 원천으로 채우지 않는다. 업종 중앙값 비교 / PBR·PEG / 3년 밴드 내 위치 / 주가 위치 판정
