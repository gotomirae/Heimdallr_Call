...
| ID | 항목 | 배점 | 산식 |
|---|---|---|---|
| P1 | 실적 발표 초과반응 | 15 | 발표 후 D+1·5·20 지수 대비 평균 −20%p→0 · +30%p→15 |
| P2 | 전망·주가 괴리 | 10 | 3·6·12M 평균 상대수익률 − 90일 이익전망 변화, −30%p→0 · +30%p→10 |
| P3 | 주가 상승 이유 | 15 | `(1+주가수익)=(1+TTM 이익성장)×(1+PER변화)`로 12M 상승을 분해. 로그 기여도상 멀티플 몫 0%→0 · 100%→15. 주가가 하락했으면 상승 원인을 만들지 않고 제외 |
| P4 | 내재 성장률 갭 | 20 | 현재 TTM PER이 3년 뒤 자기 9분기 평균 PER이 되기 위한 연 이익성장률 − 동일 EPS 기준 컨센서스 성장률, −15%p→0 · +20%p→20 |
| P5 | 자기 역사 밸류 | 10 | 현재 TTM PER의 9분기 평균 대비 할증, −30%→0 · +50%→10 |
| P6 | 피어 성장단가 | 10 | `현재 TTM PER ÷ 예상 이익성장률`의 동일 투자섹터 중앙값 대비 할증, −30%→0 · +50%→10. 유효 피어 5개 미만이면 제외 |
| P7 | 중기 상대 주가 | 10 | 3·6·12M 지수 대비 초과수익 평균, −30%p→0 · +30%p→10 |
| P8 | 단기 과열 | 10 | 현재 RSI·5일 수익률·52주 고점 접근을 등가 평균. 2개 미만이면 제외, 합성 20→0 · 80→10 |

> **낮을수록 아직 덜 반영됨.** 공개 원자료가 없는 항목은 0점으로 만들지 않고 분모에서 제외한다. 측정 가능 배점이 45점 미만이면 PRI 자체를 판정하지 않으며, 80점 미만은 숫자를 숨기지 않고 참고용 경고를 붙인다. 내재 성장률은 할인율·영구성장률을 추측한 DCF가 아니라 **자기 역사 배수로의 3년 정상화 역산**이다. PEG형 비교는 위험·자본효율을 제거하지 못하므로 시장 전체가 아닌 동일 투자섹터 안에서만 보조적으로 쓴다. 과열은 미래 매수자를 예언하지 않고 현재 가격의 단기 쏠림만 측정한다. 발표 직후 5거래일 외국인 수급은 시간축이 달라 이 축에 섞지 않는다.
...
2. **판정 카드** — 등급(★○△·) / **투자 매력도 7축 스택 바** / 실적 원점수 A·B·C·D 감사값 /
   **PRI 8축 분해 바와 내재 성장률·상승 원인·성장단가·과열 근거** / 분기 내 백분위 /
...
# PRI 3.0
PRI_NEW_WEIGHTS = {"event": 15, "revision": 10, "driver": 15, "implied_growth": 20,
                   "valuation_history": 10, "valuation_peer": 10,
                   "relative": 10, "overheat": 10}
