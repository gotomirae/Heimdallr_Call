...
[유저 메시지] ~4,000토큰
 ...
 4-1) 3단계 최종 정기보고서 공시일 기준 snapshot과 실제 5번째 KOSPI 거래일 snapshot의
  ...
  5) 주가        현재가·실적발표 반응·전망 괴리·12M 이익/멀티플 분해·내재 성장률 갭·
                 ...
                 + 네이버 기업실적분석의 최근 확정 PER·내년도 연간 (E) PER
 ...
 6-1) 최근 공시  공시명 + 접수일 (발췌가 '내용'이면 이건 '무엇이 언제 나왔는가'다).
  ...
  7) 업종 비교   동일 KRX 업종 상위 5개사의 시총·최신 평가 분기 매출·영업이익·OPM,
                 네이버/FnGuide 올해 F.PER·ROE와 내년도 F.ROE, 시총÷최근 4분기 순이익 PER
```
...
3. **분기 실적 추이 (10분기) + 실제 주간 종가** — 매출액 → 영업이익·OPM → 매출·영업이익 YoY → 수주잔고·신규수주 순서로 표시한다. 영업이익·OPM과 수주잔고·신규수주는 각각 한 그래프에 비교 표시한다. 두 YoY는 실제 %를 한 좌표에 놓고 모든 분기의 두 원값을 바로 아래에 병기한다. 결측을 선으로 잇지 않으며 흑전·적전은 상태 라벨로 표시한다. 수주 정형 수치가 없으면 미수집을 밝힌다. 주간 종가는 네이버 일봉을 같은 ISO 주의 마지막 실제 거래일 한 점으로 접어 같은 기간의 종가·주봉 MACD(12·26·9)·RSI(14)를 별도 패널로 표시한다. 화면은 아홉 지표 각각의 최신 위치·의미·다음 확인사항을 결정론적으로 설명한다.
4. **분기 히스토리 표** — 최근 10개 분기를 누락 없이 분기 · 매출/YoY/QoQ · 영업이익/YoY/QoQ · OPM · FCF · 잠정/확정 구분으로 표시
5. **컨센서스 대비** — 서프라이즈 % + 추정기관 수 (없으면 "커버리지 없음" 명시)
6. **밸류에이션** — 네이버 현재 공개값의 최근 4분기 PER·올해 F.PER·올해 ROE·내년 F.ROE를 같은 기준일로 표시하고 DART 최근 4개 분기 순이익·현재 시총 계산값은 검산으로만 병기한다. 네이버에 다음 연도 ROE가 없으면 다른 원천으로 채우지 않는다. 업종 중앙값 비교 / PBR·PEG / 3년 밴드 내 위치 / 주가 위치 판정
...
- 발굴 목록의 기본 정렬은 최신 분기 → 최종 등급 → 최신 실적·가격으로 계산한 섹터 기회 순위 →
  투자 매력도 → 낮은 PRI → 영업이익 YoY다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
  ★/○ 비율·미반영 비율·점수·이익 양(+) 비율을 사전식으로 비교하며 하나의 점수로 합치지 않는다.
  한국은행 공식 RSS의 최근 매크로 제목·링크와 적용 점검 조건을 함께 표시하되 뉴스 빈도는 점수화하지 않는다.
