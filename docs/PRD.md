...
- **LLM 분석 대상: 성장 가속 종목 전부.** 그 밖의 유형은 데이터 수집·스크리닝·차트·공시 갱신을 동일하게 수행하고 LLM 분석만 제외한다.
...
대상은 **성장 가속 종목만**이다. `(1단계) 잠정실적 발표 초기 분석`을 저장하고,
같은 분기의 정기보고서 확정치가 들어오면 `(2단계) 최종 분석`을 자동 재실행해 교체한다.
...
7. **LLM 분석** — 성장 가속 종목에만 표시. `(1단계) 잠정실적 발표 초기 분석` → `(2단계) 정기보고서 공시 후 최종 분석` 자동 갱신 / 핵심 내용 / 트리거 / 시나리오 / 리스크 / 가치-가격 비교
...
| 워크플로우 | 스케줄 (UTC) | 내용 |
|---|---|---|
| `universe_daily.yml` | `0 21 * * *` (06:00 KST) | KIND+네이버+corpCode 갱신, 시세·지수 및 전 유니버스 네이버 컨센서스/PER 수집, 정밀 재무 보충, PRI·점수 재계산 |
| `disclosure_poll.yml` | 매일 `*/30 0-14 * * *` + 야간 `17 15,18,21 * * *` | DART 폴링 → 새 정기보고서의 확정 재무·발췌 → 잠정 분석의 확정 갱신 → 스크리닝 → ⚡알림 |
...
- LLM은 성장 가속 종목만 분석한다. 저장한 재무·발췌 내용 hash와 현재 근거가 달라지면 재분석한다.
  매일 변하는 시세나 수집 시각만으로 유료 재호출하지 않는다. 레거시는 분석 이후 들어온 근거가 있을 때 갱신한다.
  월·일 비용 제한은 유지하며, 한도에 걸린 분석은 다음 실행으로 넘긴다. 기존 해석과 최신 숫자는 시점이 다를 수 있다.
