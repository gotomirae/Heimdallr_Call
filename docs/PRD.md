...
3. `(3단계) 리포트 창 종료` — 2단계 공시 뒤 **KOSPI 실제 5거래일**이 지난 시점에,
   네이버/WiseReport 컨센서스 이력이 실제로 바뀐 경우만 최종 분석

리포트 원문을 수집하는 계약은 아직 없다. 따라서 3단계는 “리포트를 읽었다”고 주장하지 않고
공개된 컨센서스 변경을 리포트 반영의 보수적 대리 신호로 쓴다. 커버리지가 없거나 값이 같으면
LLM 호출 없이 창을 닫는다. 시세·시가총액·PRI 변화만으로는 유료 분석을 다시 열지 않는다.
...
[유저 메시지] ~4,000토큰
 ...
 4-1) 3단계 변화 정기보고서 공시일 기준 snapshot과 실제 5번째 KOSPI 거래일 snapshot의
                 연간·향후 분기 컨센서스 변경값. 리포트 원문이 아니라는 출처 한계를 명시한다.
...
7. **LLM 분석** — 성장 가속 종목에만 표시. `(1단계) 잠정실적` → `(2단계) 정기보고서` →
   `(3단계) 5거래일 컨센서스 변경` 배지 / 핵심 내용 / 트리거 / 시나리오 / 리스크 /
   가치-가격 비교. 3단계 변화가 없으면 2단계가 최종 상태다.
...
| 워크플로우 | 스케줄 (UTC) | 내용 |
|---|---|---|
| `universe_daily.yml` | `0 21 * * *` (06:00 KST) | KIND+네이버+corpCode 갱신, 시세·지수 및 전 유니버스 네이버 컨센서스/PER 수집, 정밀 재무 보충, PRI·점수 재계산 → 5거래일 창이 닫힌 ★·○ 중 컨센서스 변경분만 3단계 LLM |
...
- 자동 LLM은 성장 가속 중 ★·○만 분석한다. 잠정→정기보고서→5거래일 컨센서스 변경의
  세 이벤트만 열며, 변화 없는 3단계와 매일 변하는 시세·시가총액·PRI에는 호출하지 않는다.
...
| **분기 합계** | | | `잠정 대상 + 정기보고서 대상 + 컨센서스 변경 대상`의 실호출 합계 |
...
재시도 금지, ★·○ 한정, 변화 없는 3단계 생략으로 먼저 줄인다.
