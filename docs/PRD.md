...
1. **헤더** — 종목명·코드·업종·시총 / 네이버 현재가·등락·기준일(최대 1분 캐시, 실패 시 마지막 저장일 명시) / 52주 위치 게이지 / 3·6·12M 수익률 (지수 대비 병기)
...
3. **분기 실적 추이 (10분기 + 다음 분기 컨센서스) + 실제 일간 종가** — 녹색 매출 막대와 흰색 GPM, 황금색 영업이익 막대와 흰색 OPM을 표시한 뒤 매출·영업이익 YoY → 수주잔고·신규수주 순서로 표시한다. 다음 분기 네이버 컨센서스는 `(E)`와 점선·윤곽 막대로 구분하고 전년 동분기 실적과 대조해 YoY를 계산하며 성장률의 원자료·추정기관·스냅샷일을 붙인다. 발표 분기의 영업이익 성장률 판단과 다음 분기 전망은 별개 카드로 표시한다. 두 YoY는 실제 %를 한 좌표에 놓고 모든 분기의 두 원값을 바로 아래에 병기한다. 결측을 선으로 잇지 않으며 흑전·적전은 상태 라벨로 표시한다. 수주잔고·신규수주는 DART 정기보고서와 OpenDART 공시의 단위·분기·공시 범위를 검증한 경우만 표시하고, 주요계약 합계를 전체 회사 잔고나 신규수주로 바꾸지 않는다. 주가는 네이버 일봉을 그대로 쓰며 과거 각 분기의 잠정·확정 발표일을 점선과 라벨로 모두 표시한다. MACD·Signal의 의미와 현재 교차 방향, RSI(14)의 의미와 45선 추세를 실제 최신값으로 설명한다. 주가 원인은 수치 변화와 매크로·기업 펀더멘털 사건을 출처 기반 스토리로 함께 보인다.
4. **분기 히스토리 표** — 최근 10개 분기를 누락 없이 분기 · 매출/YoY/QoQ · 영업이익/YoY/QoQ · OPM · FCF · 잠정/확정 구분으로 표시
5. **컨센서스 대비** — 서프라이즈 % + 추정기관 수 (없으면 "커버리지 없음" 명시)
6. **밸류에이션** — 네이버 기업실적분석의 **올해 PER(예상)·내년 F.PER(예상)·올해/내년 ROE·FCF·PEG**를 같은 기준일·연도축으로 표시하고 실제 수치가 의미하는 스토리를 짧게 붙인다. 네이버에 값이 없으면 다른 원천으로 채우지 않는다. 업종 중앙값 비교 / 3년 평균 PER 대비 / 주가 위치 판정
7. **LLM 분석** — 자동 분석은 성장 가속 ★·○에 적용하고, 모든 종목은 버튼으로 단건 요청할 수 있다. `(1단계) 잠정실적` → `(2단계) 정기보고서` →
...
8. **업종 비교 표** — 같은 섹터에서 주요 제품 설명 유사도를 먼저 적용한 5개사의 시총·매출·영업이익·OPM·올해/내년 ROE·올해/내년 예상 PER와 표시 종목 중앙값. 가로 스크롤에서도 종목명 열은 고정한다.
...
- 열린 대시보드는 `constants.py`의 화면 갱신 간격에 따라 서버 데이터를 다시 읽되, 클릭·입력·스크롤
  ...
  복원한다. 종목 수·필터·정렬·결측 의미는 압축 전과 같아야 한다.
- 발굴 목록의 기본 정렬은 미국 중심 글로벌 매크로에 맞는 섹터 기회 → 투자 매력도 → 낮은 주가반영도 → 낮은 내년 F.PER → 높은 내년 F.ROE → 영업이익 YoY → 최종 등급 → 최신 분기다. 먼저 누른 머리글의 우선순위·방향을 고정하고 새 열을 다음 동률 해소 규칙으로 붙인다. 각 열은 기본 → 오름차순 → 내림차순 → 기본 순서로 순환하며, 기본 복귀 시 그 열만 제거하고 다른 열의 우선순위·방향은 보존한다. 개별 ×와 전체 기본 추천 순서 복원을 제공한다. 표 바로 위에는 종목명/코드·성장 유형·등급·섹터·시총·영업이익 컨센 상회/하회·분기 필터를 둔다. 섹터 순위는 게이트 통과 3종목 이상만 대상으로
  ...
  매일 06:45 KST 사전 예열 후 07:00 KST 갱신 목표(07:15·07:30·08:00 재시도)의 별도 배치에서 미국 마지막 **완료된** 거래일의 S&P 500·나스닥·필라델피아 반도체·VIX와 미 연준의 최신 FOMC 성명을 수집·검증해 스냅샷을 갱신한다. 사전 예열이 성공해도 07시 이후 첫 실행에서 기준 시각을 다시 기록한다. 미국 AI·데이터센터
  투자와 연결된 반도체·전력 공급망은 시장 국면에 따라 우선순위를 조정하되 구조화 실적에서 ★/○가 확인된 섹터만 승격한다. 종가 출처는 Yahoo Finance와 TradingView, 통화정책은 미 연준, 물가·고용 참고 링크는 BLS로 구분하고 시장 기준일·갱신 시각을 표시한다. 영문 원문 링크는 한국어 번역 보기를 기본으로 연다.
